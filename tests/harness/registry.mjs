...
export const DOMAINS = {
  ...
  admin: {
    ...
    checks: [
      ...
      { script: 'scripts/testStudentCardLayout.mjs', builders: [], note: '2026-09-04 — 학생관리(StudentDirectory) 카드에서 "🔓 로그인 잠금 해제" 등 버튼이 여러 개 뜨는 학생의 이름이 좁은 화면에서 가려지던 문제(운영자 제보) 회귀 방지. 오른쪽 버튼 그룹이 flex-shrink-0이라 왼쪽 min-w-0 이름 블록을 밀어내며 0에 가깝게 눌렸다 — 행 컨테이너 flex-wrap 추가 + 버튼 그룹 flex-shrink-0 제거(min-w-0로 교체) + 이름 블록에 flex-shrink-0 가드 추가(CSS 클래스만, 로직/버튼 텍스트/핸들러 무변경). 412KB급 컴포넌트라 SSR 대신 renderStudentCard 함수 본문을 정적 텍스트로 추출해 클래스 문자열을 검사(testExcelHeaderGuard.mjs와 동일 관례). 수정 전 소스로 4단언 FAIL 실측(규칙 15) 후 전체 PASS. 잠금 해제 버튼 조건/텍스트 무회귀 + 규칙 11(pin_hash/pin_fail_count/pin_locked_until 등장 횟수 origin/main 대비 불변) 확인 포함. 네트워크 0, DB 접촉 0.' },
