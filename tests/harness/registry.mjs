...
export const DOMAINS = {
  login: {
    ...
    checks: [
      ...
      { script: 'scripts/testAdminPinThrottle.mjs', builders: [], extra: false, note: '2026-09-04 야간 T6b — 관리자 PIN 브루트포스 스로틀 회귀(security-reviewer 감사 Medium #P12). verify-admin-pin.js에만 있던 실패 지연(1.5초)이 checkAdminReauth(admin-pin-actions.js 12개 액션/compute-word-king.js/start-new-season.js 공유)와 clear-student-pin.js/set-student-pin.js 인라인 비교에는 없어 그 경로들로 전속력 온라인 브루트포스가 가능했던 문제 수정. api/_pinAuth.js에 공용 adminPinFailureDelay()/timingSafeStringEqual() 도입 + 4개 실패 경로 전부 적용 + verify-admin-pin.js 평문 `===` 비교를 timingSafeStringEqual로 교체. 정적 검사(ADMIN_PIN 직접 비교 파일 전부 지연 호출/평문 비교 부재) + 행동 검증(틀린 PIN은 테스트 전용 ADMIN_PIN_FAIL_DELAY_MS 오버라이드만큼 지연·응답 형태 무변경, 맞는 PIN은 즉시 반환) 23단언, 인가 실패 경로만 태워 네트워크/DB 접촉 0. dispatch 회귀(섹션4)는 ADMIN_PIN 부재 시 SKIP(기존 4개 스크립트와 동일 관례). 수정 전 소스로 8단언 FAIL 실측(규칙 15, git stash로 재현) 후 전체 PASS 전환. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      { script: 'scripts/testRlsSecurity.mjs', builders: [] },
      { script: 'scripts/testSecurityRegressions.mjs', builders: [], extra: false, note: '2026-09-04 — 보안 회귀 고정 스위트(T9): grant-xp.js 레거시 XP 분기 인증부재(KNOWN, handoff.md 2026-09-02) / AdminScreen.jsx 미인증 fetch 금지 / src 시크릿 노출 0 / .env git 이력 0 / addStudent 호출부 0(자기등록 OFF) / anon 권한 라이브 프로브(phantom id) / PIN 잠금 상수·ilike 이스케이프 / Release Gate 자격증명-부재 자가검증(공허 통과 금지). 네트워크는 anon key phantom id 프로브만, 실데이터 무접촉.' },
      { script: 'scripts/testLoginRestoreCrash.mjs', builders: ['race'] },
      { script: 'scripts/testNextFailState.mjs', builders: [], extra: false, note: '2026-09-06 야간 QA — 2026-08-08 d28709d P0(PIN 잠금 만료 후 즉시 재잠금 루프) 회귀 테스트가 registry 미등록으로 한 번도 verify:all에서 실행되지 않던 것을 등록(순수 함수, 네트워크 0).' },
      { script: 'scripts/testCiNameMasking.mjs', builders: [], extra: false, note: '2026-09-09 야간 QA — 2026-09-03 보안수정 Track 12(CI 학생 이름 마스킹, 이 저장소가 public이라 GitHub Actions 로그가 누구에게나 보이는 문제 대응)가 registry 미등록으로 verify:all/CI에서 한 번도 실행되지 않던 것을 등록. prodCheck.mjs --fixture --json 기본 마스킹/--show-names/CI 강제 + golden maskName 독립 참조 구현 계약 + release-gate.yml Gate 3b stdout에 --json 미tee 정적 검사, 32단언. --fixture 고정이라 네트워크 0.' },
      { script: 'scripts/testStudentsRlsPhase2b.mjs', builders: [], extra: false, note: '2026-09-09 야간 QA — Phase 2b Step 1-A(2026-09-02, Supabase Auth 세션 발급 플래그 + PIN 로스터 최소화 조회, api/_pinAuth.js·api/verify-student-pin.js·api/student-pin-status.js)가 registry 미등록으로 한 번도 verify:all에서 실행되지 않던 것을 등록. esbuild 번들 + 인메모리 가짜 @supabase/supabase-js(auth.admin.{createUser,generateLink,updateUserById} 포함)로 세션 발급 플래그 ON/OFF·fail-open·로스터 최소화 응답 형태·checkAdminReauth timingSafeEqual 치환 무회귀·pin_ 접두 컬럼 전역 무접촉을 87단언(전부 offline)으로 검증. 마지막 1건은 단언이 아닌 "라이브 기준선"(anon key 읽기전용 HEAD students?select=id&limit=1, RLS 전환 전 상태 확인용)으로 .env 부재 시 SKIP하지만 이 저장소처럼 .env가 있으면 실제로 프로덕션에 read-only HEAD 요청 1건이 나간다 — 이 스크립트 자체의 기존 설계이고 이번 등록에서 새로 만든 동작이 아니다(정직하게 기록, 규칙 18). 네트워크 의존 없이 이 도메인을 돌리려면 로컬에서 .env를 일시적으로 옮겨두거나 무시할 것.' },
    ],
  },
  student: {
    ...
    checks: [
      ...
      { script: 'scripts/testStarDeltaOnEntry.mjs', builders: ['race'], extra: false, note: '2026-09-06 야간 QA — 실사고 가드라 게이팅으로 승격(순수·네트워크 0, 당일 PASS 확인). 2026-08-12 — 이름 키 소유권 가드(claimName, useStudent.js loadRecord). 동일 기기·동일 표시이름의 다른 studentId가 앞서 생긴 계정의 로컬 진행도(별/스티커/캘린더)를 물려받는 실사고 재현 + 수정 검증. 신규 계정 진입 델타 0(대조군)/동명이인 가로채기 차단/재입장 중복지급 0/dedupKey 재호출 방어/실제 보상 정상 지급/다른 이름 조합 재확인 6개 시나리오, 25단언. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
  ...
  admin: {
    ...
    checks: [
      ...
      { script: 'scripts/testTextbookIsolation.mjs', builders: [], extra: false, note: '2026-09-04 — Overnight QA T2: 교재/반/유닛 격리 회귀 스위트(UUID identity, 이름 문자열 무매칭). testTextbookGradeLabel.mjs가 "관리자가 배정할 교과서를 고르는 <select> 라벨" UI만 검증했던 것과 달리, 이 스위트는 그 밑 데이터 계층(createClass/assignTextbook/setClassWords/setAssignmentUnit/setPrimaryAssignment/setPrimaryTextbook/setStudentClass/linkTextbookToClass/getStudentClassAssignments)을 직접 검증한다. 6개 시나리오: ①동저자·동출판사·다학년+유사 이름("중2 천재 이상기"/"중2 천재 이상기 2학기")+동일 유닛번호("Unit 1")여도 createClass/textbook이 UUID로 완전히 분리되고 getTextbookById/getTextbookUnits/getLearnableTextbookUnits/assignTextbook/getStudentClassAssignments 전부 id 기준으로 구분 ②반 1개↔교재 N개·교재 1개↔반 N개 다대다 연결(getClassTextbooks) + 학생 textbookOptions(App.jsx useMemo의 byId union+dedupe 로직을 그대로 재현, 실제 wordLibrary 함수 호출) 교차오염 0/dedupe 정상 ③setPrimaryTextbook이 정확히 1개만 primary로 바꾸고 건드리지 않은 다른 secondary 배정(current_unit_id)은 무접촉, setAssignmentUnit도 대상 배정 행만 갱신, students.current_unit_id는 항상 primary 교재 소속, 학습기록 테이블(student_progress/student_daily_progress)은 이 섹션 전체에서 쓰기 0건 ④setStudentClass(반 이동)가 다른 교재 SCA 행을 보존하고 student_class_assignments에 DELETE를 전혀 issue하지 않음(2026-09-03 T4 정적 감사 Med #4 demote 통일 수정의 회귀 방지 — FAIL-first로 실측: textbook-mode 분기를 delete로 임시 되돌리면 이 섹션 2개 단언이 즉시 FAIL함을 확인 후 원복) ⑤신규 학생 + 동명이인("Barry"×2)/아카이브·테스트 계정 이름 패턴(Paul_DUP_Test/QA_Alpha) 전부 UUID로 분리 배정되고 서로 새지 않음 + 6개 교재 배정 함수 전부가 .eq(\'name\'...)로 학생을 찾지 않는 정적 검사(규칙 4) ⑥setClassWords 업로드가 만드는 유닛의 textbook_id가 항상 그 반 소유 교재이고 동일 이름("Unit 1")이 두 교재에 있어도 유닛 id가 분리돼 충돌 없음 + 학생 라벨은 tb.name 그대로(학년 유추 없음 — 클라이언트 textbook 캐시 객체에 grade/grade_id 필드 자체가 없음을 구조적으로 확인, v3_13이 DB textbooks.grade_id 컬럼을 추가했지만 refreshTextbooks()의 select가 그 컬럼을 아예 읽지 않음). 하네스: wordLibrary.js esbuild 오프라인 번들 + 쓰기 기록형 인메모리 가짜 supabase(update/insert/delete/upsert 전부 기록, textbooks.name/classes.name/class_textbooks(class_id,textbook_id)/student_class_assignments(student_id,class_id) UNIQUE + uq_sca_student_textbook(student_id,textbook_id) partial unique까지 실제 SQL 제약 그대로 시뮬레이션 — supabase_v2_9_student_class_assignments.sql:92, supabase_v3_1_textbooks.sql:40,62,81-82 근거). 43단언 전체 PASS. 네트워크 0, 실제 Supabase 접촉 0.' },
      { script: 'scripts/testAdminFlows.mjs', builders: [], extra: false, note: '2026-09-04 — Track M(테스트 전용 워크트리): 관리자 핵심 흐름 복합 회귀 스위트. 개별 흐름은 이미 testCreateStudentUnitAssignment.mjs(create_student)/testTextbookIsolation.mjs(교재·반 다대다/전환/이동)/testAdminUnitEdit.mjs(유닛 저장/유령 거부)/testAssignmentUnitGuards.mjs(setAssignmentUnit/setPrimaryAssignment/setPrimaryTextbook 유령 가드)가 각각 검증하지만, 이 스위트는 그 로직을 재구현하지 않고 "학생 생성 → 반 이동 → 교재 배정/전환 → 유닛 변경"을 같은 픽스처 위에서 이어 붙여 단계 간 상호작용(예: create_student가 만든 SCA 위에서 setStudentClass가 다른 교재 SCA를 보존하는지, 교재 왕복 전환 후에도 각 SCA가 "자기 진도"를 유지하는지, 학습기록 테이블이 전 구간에서 계속 무접촉인지)을 고정한다. PART A는 testPinSetupCapability.mjs/testCreateStudentUnitAssignment.mjs와 동일하게 api/admin-pin-actions.js를 esbuild로 번들 + @supabase/supabase-js 인메모리 다중 테이블 가짜(create_student: 반+textbookId 지정 시 SCA primary+textbook_id+첫 학습 유닛 배정 / 교재 미소유 반+textbookId 미지정 시 문서화된 현재 동작인 null 배정으로 성공). PART B는 testTextbookIsolation.mjs와 완전히 동일한 쓰기 기록형 인메모리 가짜 supabase(UNIQUE 제약 시뮬레이션 포함)로 src/utils/wordLibrary.js를 오프라인 번들 — setStudentClass(class_id 변경 + 이전 primary demote 유지·삭제 아님 + 다른 교재 SCA 보존 + 목적지 유령 유닛 정규화 함정 회피), assignTextbook/removeTextbookAssignment(그 행만 삭제, primary는 이 경로로 삭제 불가), setPrimaryTextbook/setPrimaryAssignment(정확히 1개 primary, current_unit_id/unit_name 동기화, 왕복 전환에도 각 SCA가 자기 진도 유지), setAssignmentUnit(다른 교재 소속 오배정 유닛 → unit_textbook_mismatch, 유령 유닛 → unit_not_learnable), setStudentUnitById/setStudentUnit(UUID 풀 제한 저장, 미존재/유령 이름 거부) 전부 검증 + 전 구간(반이동/배정·전환·해제/유닛변경) student_progress/student_daily_progress/word_status 쓰기 0건 + 정적 검사(관련 함수 전부 .eq(\'name\'...)로 학생을 찾지 않음, create_student도 .from(\'students\') 체인 한정 검사로 units 테이블의 정당한 .eq(\'name\',unitName) 오탐 배제). FAIL-first 자가검증(규칙 15) — src를 고치지 않고 가짜 DB에 "delete-instead-of-demote" 옛 구현을 흉내낸 연산을 직접 기록해, 그 상태에서 데모트 단언 조건이 실제로 false가 됨을 증명(회귀를 스위트가 실제로 잡아냄을 소스 무변경으로 재현). 63단언 전체 PASS. 네트워크 0, 실제 Supabase 접촉 0.' },
      { script: 'scripts/testTextbookGradeLabel.mjs', builders: [], extra: false, note: '2026-09-03 — 교재 학년 라벨 오선택 방지("중1 천재 이상기" 추가 시도 → 학생 화면에 "중2 천재 이상기" 노출 실사고). 근본 원인은 데이터 계층이 아니라 순수 UI: createClass/textbooks는 정확 이름 UNIQUE 매칭만 하고 학년 유추·정규화가 전혀 없는데(정상), TextbookAssignmentPanel.jsx의 "배정할 교과서" <select> 옵션이 tb.name만 렌더해 이름이 같고 학년/출판사만 다른 두 교재를 구분할 수 없었다. 수정: src/utils/textbookLabel.js 신설(textbookLabel/textbookOptionLabel — ClassTextbookLinks.jsx가 2026-07-22부터 쓰던 인라인 tbLabel을 글자 그대로 추출, 출력 불변) → TextbookAssignmentPanel.jsx 옵션에 출판사+유닛 수 추가, AdminScreen.jsx ExcelUpload 반 선택 <select>에 교재 컨테이너 [교재] 태그 추가. createClass/assignTextbook/setClassWords 등 쓰기 경로는 전혀 무수정. 하네스: a-c는 wordLibrary.js esbuild 오프라인 번들 + 인메모리 가짜 supabase(insert/update/delete/single 지원, textbooks.name·classes.name·class_textbooks(class_id,textbook_id) UNIQUE 시뮬레이션으로 23505 idempotency 재현 — supabase_v3_1_textbooks.sql:40,62 근거)로 createClass 학년 재작성 없음/신규 행 분리/정확 이름 idempotency 검증. d/e/f는 textbookLabel.js 순수 함수 호출(출력 계약) + 두 컴포넌트·App.jsx 소스 정적 배선 검사(정규식) — TextbookAssignmentPanel은 assignments state가 useEffect 비동기 로딩 뒤에만 채워져(SSR은 effect 미실행) react-dom/server로 옵션 렌더 분기에 도달할 수 없어 정적 검사로 대체(jsdom/react-test-renderer 미도입, 규칙 6), ClassTextbookLinks는 로딩 게이트가 없어 실제 SSR로 라벨 문자열 리팩터 전후 불변을 확인. AdminScreen.jsx ExcelUpload도 다수 무관 모듈을 상단에서 import해 전체 SSR 번들 비용·위험이 국소 수정 대비 과해 정적 검사로 대체(과제 지시가 "static or SSR" 명시 허용). 규칙 15 FAIL-first — 수정 전 코드(git stash로 재현)에서 a-c(계약 동결) 13단언은 그대로 PASS, d/e/f는 src/utils/textbookLabel.js 부재로 모듈 로드 자체가 실패(ERR_MODULE_NOT_FOUND, exit 1) — 수정 후 41단언 전체 PASS. 네트워크 0, 실제 Supabase 접촉 0. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      { script: 'scripts/testAccountClassification.mjs', builders: [], extra: false, note: '2026-09-04 E/F 계정 분류 정책(Paul/Cookie/Jinaa/Barry/QA/archived) 34단언.' },
      { script: 'scripts/testOpsStatus.mjs', builders: [], extra: false, note: '2026-09-04 — 운영 자동검증 표준화(Track D). scripts/lib/opsStatus.mjs(순수 모듈, 새 파일)가 STATUS enum(PASS/WARN/FAIL/BLOCKED_NEEDS_APPROVAL)/worstStatus/severityToStatus/assertFinding(finding 스키마 검증)/recommendedActionFor·writeRequiredFor·approvalRequiredFor(INVARIANT_CODES 22종 + health CHECK_CODES 10종 + WARN 전용 파생 2종 + HEALTH_FAIL 전수 커버리지, 미상 코드는 조용한 false가 아니라 보수적 true/확인 필요 폴백)/fromProdCheckReport·fromHealthReport(scripts/prodCheck.mjs·scripts/studentHealthCheck.mjs 의 --json 출력 형식은 그대로 두고 표준 finding[]으로만 변환 — 두 판정 모듈 재구현 없음, CODE_META.recommended 재사용)/renderSummary 를 제공하고, npm run prod:report(scripts/prodReport.mjs, 새 CLI)가 그 위에서 두 하네스를 실행(또는 --from-dir로 기존 JSON 재사용)해 docs/qa/ops-report/ops-report-latest.{md,json} + docs/qa/ops-report/history/ 타임스탬프 사본을 생성한다(Executive summary/Production health/Student integrity/Textbook integrity/Garden/Paul Town/Reward/Excel/Security/Performance/Ghost·legacy/Open PRs/Approval queue 13섹션 고정 순서). prod:report는 prodHotfix.mjs를 import조차 하지 않아 구조적으로 apply 경로에 도달할 수 없다. 141단언(enum/worst/severity 13 + assertFinding 유효·필드누락 20 + 코드 커버리지 74 + 어댑터 15 + renderSummary 4 + CLI --from-dir 통합 15, 마스킹 우회 방지용 decoy 필드 회귀 포함) 전부 PASS, --from-dir 모드는 네트워크 0(gh/fetch 구조적 스킵). 라이브 1회(READ-ONLY, anon key)로 실제 docs/qa/ops-report 산출물 생성까지 확인(별도 handoff 기록). 산출물 경로는 docs/qa/ops-report/(당초 docs/qa/ops/를 쓰려 했으나 .gitignore의 레거시 ops/ 전면 차단 패턴이 디렉토리명 우연 일치로 함께 매칭해 커밋이 안 됨을 발견 — .gitignore 자체는 무수정하고 경로를 바꿔 회피). package.json(prod:report/verify:ops-status)도 이 트랙이 함께 등록. DB write 0. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      { script: 'scripts/testSqlExecutorErrors.mjs', builders: [], extra: false, note: '2026-09-09 — Management API 실행기 오류 문자열에 undici err.cause(code/message) 전파(권교빈 hotfix "fetch failed" 진단 불가 사고). 토큰 미노출 단언 포함. 네트워크 0.' },
      ...
      { script: 'scripts/testProdPlan.mjs', builders: [], extra: false, note: '2026-09-04 Harness V2 Track C — prod:plan(READ-ONLY 계획/drift/위험도/자격) 27단언' },
      { script: 'scripts/testDbIntegrityAuditPaging.mjs', builders: [], extra: false, note: '2026-09-09 야간 — dbIntegrityAudit fetchAll 1000행 PostgREST cap 오탐(word_status orphan 60건 가짜) 수정 회귀: 페이지네이션 3페이지 2075행/호출 3회/Range 진행/빈 페이지/HTTP 오류 전파. 네트워크 0.' },
      { script: 'scripts/testAdminStudentActions.mjs', builders: [], extra: false, note: '2026-09-09 야간 QA — Phase 2b Step 1-B(2026-09-02) api/admin-pin-actions.js 로스터 관리 서버 액션 7종(list_students/set_student_class/set_student_unit/set_students_class_bulk/set_student_house/set_primary_assignment/set_primary_textbook) 회귀가 registry 미등록으로 한 번도 verify:all에서 실행되지 않던 것을 등록. testCreateStudentUnitAssignment.mjs와 동일 esbuild 번들 + 인메모리 가짜 supabase 패턴의 독립 사본(쿼리당 결과 1회 캐시로 원본 fake의 이중실행 잠재 버그 수정) + pin_ 접두 컬럼 전역 무접촉 확인 85단언. 네트워크 0, 실제 Supabase 접촉 0.' },
      { script: 'scripts/testRegistryCoverage.mjs', builders: [], extra: false, note: '2026-09-09 야간 QA — registry 커버리지 결정론 린트(신규). scripts/의 모든 test*.mjs가 registry.mjs에 등록돼 있거나 명시적 allowlist(라이브 전용/동시 작업 트랙 등록 대기 등, 사유 필수)에 있는지 검사해 "test 파일을 만들고 등록을 잊는" 사고(testGardenGrowthFlow.mjs 등 여러 건이 사후 야간 감사로만 발견됨)를 생성 시점 가까이에서 잡는다. 반대 방향(registry가 참조하는 경로가 실제로는 삭제/리네임된 경우)도 검사. registry.mjs를 직접 import(순수 데이터 export, 부작용 없음)해 정적 텍스트 정규식보다 정확하게 판정. allowlist 항목 하나를 임시로 지우면 실제로 FAIL하는지 도입 시점에 수동 확인함(작업 로그 기록, 이 파일 자체 헤더에는 재현 스크립트 없음 — 회귀 시 이 파일의 섹션 2가 즉시 FAIL로 알려줌). 네트워크 0, DB 0.' },
    ],
  },
  homework: {
  ...
  quiz: {
    ...
    checks: [
      ...
      { script: 'scripts/testStudentPathContracts.mjs', builders: [], extra: false, note: '2026-09-04 야간(overnight T8, P8) — 학생 핵심 경로(로그인→대시보드→교과서/유닛 선택기→GuidedSession/WordDetail→퀴즈→SpellingQuestion/SpellingReview→정원(EnglishGarden)→Paul Town→로그아웃) SSR/정적 계약 스위트. 실제 화면 컴포넌트 10개(StudentSelect/Dashboard/GuidedSession/WordDetail/QuizGame/SpellingQuestion/SpellingReview/EnglishGarden/PaulTown/TextbookSelector)를 esbuild로 번들해 react-dom/server(SSR)로 렌더 — 브라우저/DB 전용 모듈(speech/paulReactions/useStudent/browserDetect/useMicReady/InAppBrowserNotice/wordLibrary/config·features/supabaseClient)만 파일 기반 가상 스텁(export let STATE + __configure, scripts/wordLibraryRaceStub.mjs와 동일 관례)으로 교체하고 나머지(정원/Paul Town 엔진, 모자 카탈로그, dailyRitual 등)는 실제 소스 그대로 번들한다. PIN 없음/busy 플래그/로그아웃 확인 등 클릭 이후 상태나 순수 텍스트 배선은 testAdminUnitEdit.mjs D/E 섹션과 동일한 정적 소스 정규식 관례로 보완. 61단언(SSR+정적) 전체 PASS: PIN 컬럼 원문 미노출(규칙 11)/교과서 라벨 name·name(publisherName) 계약(App.jsx, grade 추론 없음)/유령 유닛 학생 유닛 셀렉트 비노출/0단어 빈 상태 문구/정원 16칸+house 30포인트 잠금·해금 텍스트 전환/Paul Town paulTownBuildings 플래그/퀴즈 4지선다/연타 방지 busy 플래그 5개소. 네트워크 0, DB 접촉 0. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      { script: 'scripts/testNavHistoryModel.mjs', builders: [], extra: true, note: '2026-09-09 — 모바일 Back 순수 상태기계 프로토타입(대안 A) 59단언 — 제품 코드 무관, 설계 검증용(docs/design/prototypes). 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      { script: 'scripts/testSpellingImeGuard.mjs', builders: [], extra: false, note: '2026-09-09 — SpellingQuestion Enter 핸들러 isComposing 가드 정적 계약 12단언(IME 조합 중 제출 차단), EntranceTest 패리티. 네트워크 0.' },
      { script: 'scripts/testImeEnterGuards.mjs', builders: [], extra: false, note: '2026-09-09 — 한글 IME 입력 가능 텍스트 입력의 Enter 핸들러 isComposing 가드 중앙 계약(로그인 이름/문장 빈칸/관리자 이름 변경 + 기존 SpellingQuestion/EntranceTest). 숫자 PIN/버튼형 핸들러는 무변경 스냅샷. 네트워크 0.' },
      { script: 'scripts/testEntranceTestPadding.mjs', builders: [], extra: false, note: '2026-09-09 — EntranceTest running/result 단계 pb-24(고정 SpeedBtn 겹침) 정적 계약 6단언. 네트워크 0.' },
      { script: 'scripts/testWritePracticeAlwaysMixed.mjs', builders: [], extra: false, note: '2026-09-09 — 쓰기 연습(studyMode write)만 반 설정과 무관하게 항상 mixed 50:50 출제(운영자 결정). resolveSessionSpellingDirection 순수 함수 단위 + App.jsx mixedDirections 배선 정적 계약 + guided/review 메모 무변경 + SpellingQuestion 질문 단계 정답/반대언어 미노출 재확인. 네트워크 0, DB 0.' },
    ],
  },
  writing: {
    ...
    checks: [
      ...
      { script: 'scripts/testSpellingReviewIndex.mjs', builders: [], extra: false, note: '2026-09-09 야간 — SpellingReview 문제 번호/방향 인덱스 드리프트 회귀(classWords에 없는 id가 큐에 남는 경우) 12단언. 네트워크 0.' },
      { script: 'scripts/testClassSettingsResilience.mjs', builders: ['wordlibOffline'], extra: false, note: '2026-09-09 야간 — refreshClassSettings 회복력: 컬럼 폴백을 42703에만 게이팅, 일시 오류 시 이전 반 설정 캐시 유지(전부 기본값 리셋 방지) 3케이스 8단언. 오프라인 번들, 네트워크 0.' },
      { script: 'scripts/testColumnProbeMemo.mjs', builders: ['wordlibOffline'], extra: false, note: '2026-09-09 야간 — refreshStudents/refreshClassSettings 42703 tier 메모(페이지 로드당 1회 프로브, 반복 400 노이즈 제거) 6케이스. 오프라인 번들, 네트워크 0.' },
      { script: 'scripts/testWordSlugParity.mjs', builders: [], extra: false, note: '2026-09-09 야간 — wordLibrary/assignmentPlanner/EntranceTestAdmin의 wordSlug 3중 사본 바이트 동일성 + 실행 결과 동일성 9단언(주석이 요구하는 수동 동기화 계약을 테스트로 고정). 네트워크 0.' },
    ],
  },
  speaking: {
  ...
  unitSwitching: {
    ...
    checks: [
      ...
      { script: 'scripts/testWordLibraryPagination.mjs', builders: ['wordlibOffline'], extra: false, note: '2026-09-06 야간 QA — 실사고 가드라 게이팅으로 승격(순수·네트워크 0, 당일 PASS 확인). 2026-08-12 — words 1000행 절단(P0) + 유닛 임의 폴백 회귀. 실사고: words가 1093행이 되자 range 없는 조회가 1000행만 받아 93개 단어(24개 유닛의 position 38~49)를 에러 없이 버렸고, 40단어 유닛이 38개로 출제됐다. 함께 getClassWords의 `units.find(name) || units[0]` 무음 폴백(요청한 유닛이 없으면 조용히 첫 유닛 단어를 반환)도 제거. 스텁이 PostgREST 1000행 상한을 실제로 강제하므로 페이지네이션이 없으면 반드시 FAIL한다(수정 전 소스로 20단언 FAIL 실측 확인, 규칙 15). 절단 0/동명 유닛 비혼입/Song·Luke 배정 유닛 정확 조회/기존 경로 무회귀 6시나리오, 48단언. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      ...
      { script: 'scripts/testExcelImportFixtures.mjs', builders: [], extra: false, note: '2026-09-04 야간 QA T3 — 업로드 파서(parseExcelRows) fixture 스위트, test-only. canonical(Unit|Word|Meaning)/legacy alias(No·번호/단어/뜻) 헤더 + 선택 컬럼 4종(example/exampleTranslation/POS/cefr, "POS" 대문자 인식 확인) + 미지원 extra 컬럼("pronunciation") 무시 확인, Unit 표기(Unit1/Unit 1/1/유닛 1) 4종 비정규화(원값 그대로 별개 라벨 유지, 실DB 45행이 이미 이 표기 둘을 서로 다른 유닛으로 쓰고 있어 의도된 설계 — 자동 병합은 더 위험) 명시적 고정, 40/5/45행 절단·패딩 없음, invalid 8종(English/Korean 헤더→헤더로 정상 처리, 헤더 없음+제목행 뒤 헤더유사행 배제, unit 칸 숫자만→원값 유지, 빈 unit 칸→Unit 1 폴백, 유닛 내 중복 단어→dedupe 없음, 빈 word/meaning 행→조용히 filter, 공백뿐 셀→trim 후 빈 값 취급, 헤더 행 중간 반복→header-residue-row 경고). "Unit" 1단어 유령 유닛 재발 방지 핵심 회귀(유닛 칸="Unit"→"Unit 1" 폴백, 실단어는 보존) 재확인 + UI 경고 배선(disabled 저장 버튼) 정적 확인. 54단언 전체 PASS(FAIL 없음 — 이 파일이 발견한 결함은 없음). 별도로 gap() 리포트 5건(WARN, exit code 미반영)을 남긴다: ① Unit 표기 미정규화(설계 트레이드오프로 판단, 수정 불요), ② 헤더 없음 시 내용 무검증 위치 추정(기존 numeric-word/constant-number-column 경고로 부분 커버, 사전 기반 완전 검증은 범위 밖), ③ unit 칸 숫자만→원값 유지(①과 동일 계열), ④ 파일 전체 unit 칸 공백이어도 경고 없음(제안: unit 헤더 감지+전행 공백일 때만 좁게 경고 추가 — 미구현), ⑤ 유닛 내 중복 단어 dedupe/경고 없음(동음이의어 오삭제 위험으로 자동 dedupe 부적절 — 미구현). 1건은 FAIL로 기록: 빈 word/meaning 행이 `.filter(r=>r&&r.word&&r.meaning)`에서 완전히 silent하게 제외되어 result.warnings/카운트 어디에도 신호가 안 남는다 — 최소 수정안(빈 셀만 있는 부분 결손 행에 한해 empty-cell-row 경고 추가, 완전 공백행은 트레일링 blank row가 실제 파일에 흔해 오탐 방지를 위해 제외) 문서화만 하고 구현하지 않음(UX 회귀 위험 + test-only 세션 범위 밖 — 운영자 확인 후 별도 커밋 권장). 네트워크 0, Supabase 0, DB 무접촉. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
    ],
  },
  persistence: {
  ...
  attachment: {
    ...
    checks: [
      ...
      { script: 'scripts/testStdoutFlushOnExit.mjs', builders: [], extra: false, note: '2026-09-04 야간 P11 트랙 — 등록 편입. 스크립트 자체는 2026-09-04(CI Gate 3 stdout 절단 사고, run 33787307588) 작성돼 package.json 에 verify:stdout-flush 로 이미 있었으나 이 registry 에는 빠져 있어 verify:all 에서 한 번도 실행되지 않고 있었다(야간 감사에서 발견). studentHealthCheck.mjs 가 큰 JSON(~179KB) console.log 직후 process.exit() 를 호출해 Linux PIPE 비동기 write 가 drain 되기 전에 프로세스가 죽어 부모(verifyRelease.mjs Gate 3)가 잘린 stdout 을 JSON.parse 하다 실패하던 사고의 회귀 방지 — process.exitCode + 자연 종료 패턴으로 고쳤다. 1절(행동, spawnSync 로 2MB+ 페이로드 무손실 전달 확인 — 원본 절단은 Linux 전용이라 Windows 로컬에서는 재현되지 않을 수 있음을 스스로 명시)/2절(정적, studentHealthCheck.mjs/prodCheck.mjs/prodHotfix.mjs 3개 파일에 drain 미대기 process.exit( 이 큰 JSON 출력 이후 남아있지 않은지 정규식 확인) 총 8단언. testReleaseGate.mjs 와 마찬가지로 attachment 도메인에 배치(직접 연관은 없으나 Release Gate 계열 정적 하네스가 이미 모여 있는 위치) — extra:false 로 verify:all exit code 에 실제로 반영된다(가짜 PASS 금지). 이 등록 자체는 registry 소유 트랙(P11)이 관례에 따라 대신 수행(원본 파일 무수정). 네트워크 0(spawnSync 자식 프로세스 통신만), DB 0.' },
      ...
      { script: 'scripts/testPaulTownProgression.mjs', builders: ['race'], note: '2026-09-04(TEST-ONLY 세션) — Paul Town 진행(월드 6단계/정원 16칸 텃밭/마을 3건물/모자 8종/성장앨범 밀스톤 3종/폴의 집 소품 6종) 경계·연결·persistence·멱등·학생 시뮬레이션·UI 게이팅 통합 회귀(222단언). 경계: 모든 unlockable을 threshold-1/=/+1로 3중 검사(WORLD_STAGES/TOWN_PLACES/HAT_THRESHOLDS/CLEARED_MILESTONES 등 실제 export 상수를 그대로 순회 — 숫자 하드코딩 최소화). 연결: WORLD_STAGES가 minPoints 오름차순으로 정렬돼 있고(구조로 순서 보장) points 0..300 전 구간에서 잠금해제 구역이 항상 사다리의 접두사(prefix)를 이룸을 301회 순회로 고정, 마을 3건물(museum/library/clockTower)이 world 3단계(house/library/village)와 같은 gardenPoints 축이라 임계값 동치임을 재확인, nextStage가 각 경계에서 정확한 다음 단계를 가리킴. persistence: 실제 useStudent.js(esbuild race bundle)로 35단어 학습+모자2+밀스톤3을 만들고 getLocalRecordRaw로 실제 저장된 raw record를 "cloud progress_data"로 JSON 직렬화 → normalizeRecord/mergeProgressRecords(빈로컬,cloud)/이중 재수화(cloud,cloud)/재재수화(merged,cloud) 전부에서 gardenPoints·정원칸·hatInventory·milestones·equippedHatId가 canon(정렬된 파생 상태 JSON) 기준 byte-identical임을 확인, 구버전 백업(cleared+history만)도 throw 없이 정규화되고 clearedCount 폴백과 world 상태가 일치. 멱등: 퀴즈/철자 중복 정답·grantHats/addMilestones 중복 호출·두 기기 병합 시 같은 키 다른 시각(더 이른 시각 채택)·모자/밀스톤 지급이 totalStars/rewardLedger에 전혀 영향 없음·이미 소유한 모자는 evaluateHatUnlocks가 재반환하지 않음. 학생 시뮬레이션 A(신규 0포인트)/B(heavy 295포인트, clearedCount 210은 cleared⊆clearedWords로 별도 축 유지, 모자3소유 상태에서 crown 신규후보/chef 재지급안됨)/C(29포인트→14칸 seed, house 잠김 nextStage 1남음)/D(160포인트, village 잠금해제/kingdom 잠김, 모자2·밀스톤4 보존) 각각 실제 record를 normalizeRecord+mergeProgressRecords 왕복까지 통과. UI: EnglishGarden.jsx/PaulTown.jsx 정적 배선(computeWorldState/gardenPlots 사용, attachmentWorldFull은 라벨 텍스트 분기 1곳뿐, library canEnter가 attachmentBookshelf 요구) + esbuild+react-dom/server SSR로 실제 EnglishGarden을 points=29/30으로 렌더해 "나의 집" 행이 🔒잠김 문구 ↔ 잠금해제 문구로 실제로 갈리는지 HTML 검증(SSR 실패 시 WARN으로 대체하도록 설계, 이번 실행은 SSR 성공 WARN 0). 발견된 결함 0건(TEST-ONLY 세션 — src/ 무수정). 네트워크 0, DB 접촉 0(wordLibraryRaceStub 전량 스텁).' },
  ...
  rewardSystem: {
    ...
    checks: [
      ...
      { script: 'scripts/testTownShop.mjs', builders: [], extra: false, note: '2026-09-06 — STAR SPENDING vertical slice Phase 1(플래그 townShopV1=false, 미배포). 클라이언트 순수 모듈 src/utils/townShop.js(shopItemState 4상태·applyPurchaseResult 멱등 리듀서·normalizeShopState·purchasedDeco) + 정적 배선(features 플래그 기본 false, wordLibrary fetchTownShopState/postTownPurchase가 studentId/price를 body에 절대 넣지 않고 토큰 없으면 네트워크 0으로 relogin_required, useTownShop in-flight ref 가드, useStudent.js에 townInventory/townShop 문자열 0건 = record 무변경) + esbuild SSR로 PaulTown 플래그 OFF 렌더가 prop 미전달 렌더와 byte-identical·"책상 램프" 0건, 플래그 ON 시 구매/부족(⭐ N개 더 필요)/구매 중/보유 중(+💡 소품)/재로그인 안내 분기 확인. 75단언, 네트워크 0. 소유권·잔액 진실 원천은 서버 star_purchases/reward_totals — 클라이언트는 표시만.' },
      { script: 'scripts/testTownShopServer.mjs', builders: [], extra: false, note: '2026-09-06 — STAR SPENDING Phase 1 서버: api/grant-xp.js에 action purchase_town_item / get_town_shop_state 추가(새 api 파일 0, Vercel 12/12 한도). 토큰 sid만 학생 식별(req.body.studentId/price/starsSpent/balance 미참조를 주석 제거 소스로 정적 확인), 토큰 없음→relogin_required, 서명 불량/만료→unauthorized, RPC 42883/PGRST202/42P01/PGRST205→table_missing, 기타→rpc_failed, available 음수 노출 0. 인메모리 fake supabase.rpc가 SQL 계약(student→item→already_owned→insufficient→insert, unique 위반→already_owned)을 시뮬레이션: 충분/정확히 60/부족/중복/더블클릭/재시도/가격 조작/타 학생 body 조작/타 학생 토큰/상태 조회/town_items 조회 실패 폴백 + 기존 ledger:reward·XP 분기 회귀. 47단언, 네트워크 0. 정직한 경계: 실제 원자성은 supabase_v3_47 SQL 함수에 있고 여기서는 실행하지 않음(파일 헤더 참고).' },
      { script: 'scripts/testLegacyRewardServer.mjs', builders: [], extra: false, note: '2026-09-06 — STAR SPENDING Phase 2(레거시 서버화): rewardEngine.js에 레거시 6종(pronunciation 1/mission-clear 3/daily-mission-bonus 10/spelling-combo 1·2·3/sticker-duplicate 20/matchgame 4) 화이트리스트·소스 패턴·일일 상한·parseLegacyDedupKey·rewardVariantFromSource 추가(기존 7종 값 동결 단언). api/grant-xp.js reward 분기는 variant 추출 1줄만 변경. 서버 e2e(esbuild fake supabase): 6종 정상 지급·서버 조립 idempotency_key·재요청 duplicate:true·req.body.stars 무시·12종 invalid sourceId 거부·6종 일일 상한(matchgame 6번째 거부)·pronunciation-unidentified unknown_reward_type·V1/XP 회귀 + 단어 슬러그 특수문자(아포스트로피/괄호/물결/한글, production 27/1975=1.4% 실측) 수용·공백/콜론 거부. 122단언, 네트워크 0.' },
      { script: 'scripts/testLegacyGrantCoverage.mjs', builders: [], extra: false, note: '2026-09-06 — STAR SPENDING Phase 2 클라이언트: grantReward(별 단일 변경점) 안에서 parseLegacyDedupKey→postRewardEvent 후킹(V1 uuid 키는 null→이중 POST 없음), grantSticker(sticker, giftKey)로 sticker-duplicate 키를 timestamp+random에서 안정 키(round:signature/milestone:n/badge:threshold)로 교체(금액·조건 무변경). 실제 useStudent(esbuild race bundle+fakeReact)로 레거시 6경로 각각 정상 +금액·POST 정확히 1회, 재진입 +0/POST 0, 새로고침(재마운트) +0/POST 0, 학생 간 sourceId 안정성, 서버 실패 시 로컬 지급 보존·throw 없음, V1 앵커 6종 POST 1회씩(12/13 서버화, pronunciation-unidentified는 의도적 0), 같은 giftKey 중복 스티커 1회/다른 giftKey는 매번(레거시 보존). 60단언, 네트워크 0.' },
      { script: 'scripts/testRewardPostQueue.mjs', builders: [], extra: false, note: '2026-09-06 — postRewardEvent 내구성 재시도 큐(wordLibrary.js, localStorage paul_easy_reward_post_queue): key 멱등 enqueue, 300건 trim, 8회 실패 시 폐기, ok/duplicate/daily_cap_reached/확정 거부 6종 → 제거, 네트워크 실패·non-2xx·table_missing·unauthorized·dup_check_failed·cap_check_failed → 보존+attempts, flush는 토큰 필수·setSessionToken 시·online 이벤트 시·성공 후, localStorage 예외 시 throw 없음, 정적 사용 확인. 49단언, 네트워크 0.' },
      { script: 'scripts/testRewardRetryExactlyOneRow.mjs', builders: [], extra: false, note: '2026-09-10(QA 감사, qa/reward-audit-2026-09-10) — 클라이언트 재시도 큐(wordLibrary.js)와 실제 서버 핸들러(api/grant-xp.js, 무수정)를 처음으로 끝까지 연결: testRewardPostQueue.mjs는 큐 정책만 가짜 fetch 리터럴로, testRewardServerHardeningBehavior.mjs/testRewardCapRace.mjs는 실제 핸들러만 큐 없이 검증해 왔던 공백을 메운다. wordLibrary.js/api/grant-xp.js 둘 다 esbuild로 무수정 번들(이 파일 전용 인메모리 가짜 Supabase, testRewardServerHardeningBehavior.mjs 패턴 재사용) + 전역 fetch를 실제 handler(req,res) 직접 호출 브리지로 교체(HTTP 자체를 만들지 않음). 시나리오: (1) 시도 1 네트워크 순단(reject)으로 큐에 남음 → flush(시도 2)가 실제 핸들러로 성공 → 서버 원장 정확히 1행 → 3번째(reload 재현) 호출도 duplicate:true로 여전히 1행, 큐 최종 빈 상태. (2) 대조군 — 시도 1이 HTTP 500(insert만 강제 실패시키는 전용 훅으로 select 단계는 건드리지 않음)이어도 동일한 최종 결과. 18단언, 네트워크 0, 실제 Supabase 접촉 0.' },
      { script: 'scripts/testRewardDayBoundaryDeviceClock.mjs', builders: [], extra: false, note: '2026-09-10(QA 감사) — 하루 경계/기기 시계 오차(device clock skew) 계약 문서화(순수 함수, 정책 주장 없음 — "현재 계약(문서화)" 라벨). 핵심 대비: reward 날짜 토큰(rewardEngine.js REWARD_SOURCE_RULES pattern date/date:token/date:streak, isValidRewardSource)은 DATE_TOKEN_RE 형식만 검사해 미래/과거 어떤 날짜든 통과(오늘/어제/내일/±25h/±365일 전부 통과 확인, 말형식 위조는 대조군으로 여전히 거부) — 무제한 반복은 이 파일 소관이 아닌 rewardDailyCap(건수 상한)이 방어. 반면 XP 기간키(paulRankShared.js isValidDayPeriodKey)는 형식뿐 아니라 서버 시각 기준 ±DAY_KEY_TOLERANCE_MS(소스에서 읽은 리터럴 172800000ms=2일) 이내인지까지 확인(경계값 정확히 통과, 1ms 초과 거부 양방향). 같은 단어의 연속 이틀 wrong-word-recovered 토큰이 둘 다 개별 유효함을 "설계"로 명시(재지급 버그 아님). api/grant-xp.js L3 상한 창이 gte(created_at, kstDayStartMs(Date.now()))로 서버 자신의 시각만 쓰고 req.body를 전혀 참조하지 않음을 정적 확인 + kstDayStartMs KST 자정 경계 무회귀(무수정 확인용). 49단언, 네트워크 0, DB 0, product 코드 무수정.' },
      { script: 'scripts/testTownShopSql.mjs', builders: [], extra: false, note: '2026-09-06(사전 커밋 독립 리뷰) — supabase_v3_47_town_shop.sql/ROLLBACK 정적 단언 + 구매 함수 인메모리 시뮬레이션. 스키마(price>0, stars_spent>0, unique(student_id,item_id), idempotency_key 컬럼 없음, FK cascade), RLS/GRANT(town_items anon SELECT만, star_purchases 정책 0·GRANT 0·REVOKE), 함수(security definer, set search_path, #variable_conflict use_column, revoke public/anon/authenticated + grant service_role, notify pgrst), 권위(함수 본문에 total_stars 0회, reward_totals rt/sum(sp.stars_spent) alias), 동시성 순서 증명(pg_advisory_xact_lock이 earned/spent/already_owned/insufficient/insert보다 앞, 잠금 키에 p_student_id, unique_violation→already_owned, reason 집합 정확), 롤백(모든 DELETE WHERE, shop-lamp 한정, 함수 revoke+drop function if exists, reward 계열 테이블 쓰기 0 — read-only count 검증 SELECT는 허용). 시뮬레이션 8종: 정상/중복/정확히 60→0/부족/서로 다른 아이템 동시 구매 시 잠금으로 overspend 불가(무잠금 대조군은 overspend 발생)/학생 간 독립/total_stars 구조적 미참조/음수 하한. 네트워크 0, SQL 실행 0.' },
      { script: 'scripts/testPaulDollarSql.mjs', builders: [], extra: false, note: '2026-09-08 — Paul Dollar 2재화 분리(v3_49, 미실행). ⭐ 별 = reward_totals 누적 성취(구매로 감소 0), 💵 달러 = dollar_ledger 단일 원장(SUM, $0 시작, 레거시 변환 없음). 정적: dollar_rules 12종 시드(legacy-baseline 제외)·dollar_ledger(unique idempotency_key, RLS 정책 0·GRANT 0)·dollar_balances 뷰·AFTER INSERT 트리거 fn_reward_ledger_to_dollars(stars_delta>0·규칙 존재 시 1★=$1, exception 래핑으로 별 삽입 보호)·star_purchases는 무수정(레거시 별 시대 이력 보존, Paul QA 1행) — 달러 구매는 신규 town_purchases(unique(student_id,item_id), RLS 0/GRANT 0)에 기록·소유권은 두 테이블 합집합·town_items price_currency 기본 dollars·price 60 유지·RPC 2개 재정의(dollars 차감만, item_not_purchasable, town_purchases+음수 dollar_ledger 두 INSERT 동일 트랜잭션, total_stars 미참조)·권한·롤백이 v3_47 본문 복원+WHERE. 시뮬레이션: 지급→달러 동액, 중복 키 0, legacy-baseline $0, 별 많고 달러 부족→insufficient(별로 결제 불가), 구매 후 별 불변·달러 −60, 재시도/더블 already_owned 1행, 다른 아이템 동시 구매 직렬화, Paul QA stars 행 보존. 네트워크 0, SQL 실행 0.' },
      { script: 'scripts/testCutoverReconcile.mjs', builders: [], extra: false, note: '2026-09-07 — CUTOVER RACE fence(서버). 전역 T 스냅샷 baseline(total_stars(T)−earned(T))은 클라이언트 업로드/서버 삽입 두 시각이 비동기라 in-flight 이벤트를 이중 계산·누락 — 학생별 정지점 reconcile RPC(reconcile_legacy_baseline)로 교체. api/grant-xp.js action reconcile_legacy_baseline(토큰 sid만, snapshotTotal 정수 0~100000 검증). 실제 핸들러 번들 + 공유 인메모리 fake(ledger:reward 삽입·reward_totals 파생·RPC 계약 미러·학생 단위 직렬화)로 Device 프로토콜(grant/flush/upload/hold/reconcile) 12시나리오: 레거시만/이벤트 전/in-flight(대조군: 구 설계 직접 RPC → 320≠310 이중 계산 재현, 새 프로토콜 → 310)/보류 중 지급/이후 지급+재시도/재실행+타 기기/두 학생 교차/nothing_to_reconcile/review 라우팅/조작(타 학생 body·잘못된 스냅샷·토큰 없음·RPC 부재)/동시 reconcile 1행/기존 액션 회귀. 매 시나리오 earned == 실지급 합. 63단언, 네트워크 0.' },
      { script: 'scripts/testCutoverClient.mjs', builders: [], extra: false, note: '2026-09-07 — CUTOVER RACE fence(클라이언트). useStudent.js reconcile effect: restoreChecked 후 hold → 이 학생 큐 강제 드레인(ack 대기, 최대 3회) → 진행도 업로드 → pending 0 확인 → snapshot=record.totalStars → postReconcileLegacyBaseline → 확정 사유(reconciled/already/nothing/review/invalid_snapshot)만 localStorage 마커 → release+flush. hold는 localStorage 키로 cross-tab 적용(30초 만료). 실제 useStudent 번들+계측 스텁: 정확히 1회 호출·순서(hold→drain→sync→reconcile→release), 드레인 실패 시 skip·마커 없음, 타 학생 큐는 무관, 보류 중 지급은 snapshot 제외·release 후 1회 POST, 마커 있으면 무호출, table_missing/network는 재시도·review는 종결, 토큰 없음, cross-tab hold, grantReward dedup/V1 회귀. 네트워크 0.' },
      { script: 'scripts/testBaselineV2Sql.mjs', builders: [], extra: false, note: '2026-09-07(재작성) — supabase_v3_48_reward_legacy_baseline_v2.sql = 학생별 reconcile_legacy_baseline(uuid,int) RPC(전역 T 스냅샷 폐기 — CUTOVER RACE 해결). 정적: security definer/set search_path/#variable_conflict/revoke public·anon·authenticated + grant service_role, 순서(학생 존재→스냅샷 범위→학생 단위 advisory lock→earned→v2 존재→SNAPSHOT_SLACK→delta≤0 0행 마커→history 기반 타당성(TOLERANCE 100·MAX_INDIVIDUAL 1500) review 라우팅→insert unique→reconciled), 상수가 scripts/lib/baselineV2Guards.mjs와 리터럴 동기화, EXACT 프리플라이트(v1 마커·3테이블·reward_totals 비어있지 않음), sp.total_stars 읽기는 SNAPSHOT_SLACK 상한 검사 1회만, reward_ledger UPDATE/DELETE 0, 모니터링 뷰 GRANT 0, 롤백은 v2 행·review·cutover 마커·함수·뷰만. 시뮬레이션 13종: baseline 설치/이벤트 전·후/보류 후 release/재시도 멱등/재reconcile already/두 학생 교차/SLACK 초과 review/타당성 초과 review/MAX 초과 review/0-delta 마커/조작(bounded once)/evaluateReconcile 경계 — 매 시나리오 earned_after == 실지급 합. 네트워크 0, SQL 실행 0.' },
      ...
      { script: 'scripts/testDoubleEvents.mjs', builders: [], extra: false, note: '2026-09-04 야간 T4 — 동일 이벤트 2회 발생(더블파이어) 멱등 스위트. 학습 행동 하나가 정확히 같은 이벤트로 두 번 들어와도(더블탭/네트워크 재시도/새로고침·재로그인 remount) 별/XP/정원/원장/마일스톤 중 어느 것도 두 배로 부풀지 않는지 8개 보상 카테고리 전체를 한 파일에서 통합 확인 + 서버 원장 vs UI 표시값(stars) 대조. fakeReact.mjs + esbuild로 직접 번들한 useStudent.js(자체 캡처용 wordLibrary 스텁, testMissionBonusIdempotency.mjs와 동일 패턴이라 별도 race 번들 불필요) + 서버 섹션은 testRewardServerHardeningBehavior.mjs와 동일하게 api/grant-xp.js를 인메모리 가짜 Supabase 위에서 그대로 구동. 45단언, 네트워크 0, 실제 Supabase 접촉 0, production 요청 0. DEVELOPER_GUIDE.md 2026-09-03 규칙(신규 verify 스크립트는 기본 required)에 따라 extra:false로 등록.' },
      { script: 'scripts/testRewardDailyCeilingTable.mjs', builders: [], extra: true, note: '2026-09-09 — 결정 지원용(non-gating), 상한(REWARD_DAILY_CAP)×실제 최대 금액 천장 테이블. testRewardServerHardening.mjs 7절(~110-112행)의 worst=Σcap×REWARD_STARS=766<200 단언은 2026-09-06 레거시 6종 흡수 전 기준으로 stale(그 파일은 무수정, 운영자 결정 보류)인데다, REWARD_STARS[\'spelling-combo\']/[\'streak-bonus\']가 가변 금액 placeholder(0)라 Σ 공식 자체가 실제 최댓값(LEGACY_SPELLING_COMBO_BONUS 최댓값 3, STREAK_BONUS 최댓값 5)을 과소집계한다 — 반영 시 진짜 구조적 천장은 951(766+185). 상한 예산 자체를 확정하지 않고(운영자 sign-off 대상 그대로 유지) rewardEngine.js 상수로부터 계산 가능한 정확한 숫자만 스냅샷 고정. REWARD_DAILY_CAP/REWARD_SOURCE_RULES 키 집합 완전 일치 + legacy-baseline 상한 없음/무효 + 타입별 cap/maxStars 스냅샷 12종 + Σ 대조(766 재현 vs 951 실제) 48단언. 네트워크 0, DB 0, product 코드 무수정. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      { script: 'scripts/testRewardCapRace.mjs', builders: [], extra: true, note: '2026-09-09 — 결정 지원용(non-gating), 일일 상한(L3, api/grant-xp.js ~424-457) check-then-insert TOCTOU 레이스 관측. 상한 검사가 SELECT count → count<cap이면 INSERT 구조라 (student_id,reward_type,day)에 DB 유일성 제약이 없다(유일성은 idempotency_key 하나뿐, 같은 이벤트 재시도만 방어) — 서로 다른 sourceId(=서로 다른 idempotency_key)를 가진 동시 요청 N개가 count=cap-1에서 겹치면 전부 통과해 최대 N-1건 초과 가능. testRewardServerHardeningBehavior.mjs §7은 순차 호출만, §3의 Promise.all은 같은 이벤트(같은 sourceId) idempotency만 다뤄 이 레이스는 등록된 어떤 테스트도 다루지 않았다. 독립 인메모리 가짜 supabase(그 파일의 런타임 생성 산출물을 재사용하지 않고, 실제 handler api/grant-xp.js는 esbuild로 무수정 그대로 번들)를 자체 구현 — cap-count SELECT가 배리어(barrierSize)만큼 모일 때까지 resolve를 보류해 "N개 동시 요청이 서로의 INSERT를 못 보는 창"을 결정론적으로 재현(값 자체는 호출 시점 스냅샷이라 정직, 응답 타이밍만 겹침 — 헤더 주석 "정직한 TOCTOU 모델링" 참고). 순차 대조군(배리어=1, 회귀 가드: N회 중 정확히 1건 성공) / 동시 5건 경합(관측: 5건 전부 성공, overshoot 4 — 콘솔에 관측치 출력) / KNOWN GAP 단언(successCount>1, DB 가드 도입 시 뒤집혀야 함을 주석에 명시) / 같은 이벤트 동시 8회는 여전히 원장 1건(idempotency 대조군, 무회귀) 9단언. production 요청 0, 실제 DB 접촉 0, api/grant-xp.js·rewardEngine.js·useStudent.js·scripts/testRewardServerHardeningBehavior.mjs 전부 무수정. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
    ],
  },
