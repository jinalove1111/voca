...
export const DOMAINS = {
  ...
  rewardSystem: {
    ...
    checks: [
      ...
      { script: 'scripts/testBaselineV2Sql.mjs', builders: [], extra: false, note: '2026-09-06 — supabase_v3_48_reward_legacy_baseline_v2.sql 정적 단언(마커 가드/ON CONFLICT/source_id v2 한정/v1 무수정/롤백 WHERE/가드 상수가 scripts/lib/baselineV2Guards.mjs와 리터럴 동기화/v1 마커·reward_totals 0행 EXACT 가드) + DO 블록 인메모리 시뮬레이션(정상 라우팅·재실행 no-op·ON CONFLICT 중복 0·음수 제외·타당성 초과 review 라우팅·학생별 ledger+baseline==total_stars·BOUNDED 4종 각각 abort·EXACT 가드 abort). 네트워크 0, SQL 실행 0.' },
