...
export const DOMAINS = {
  ...
  attachment: {
    ...
    checks: [
      ...
      { script: 'scripts/testGardenGrowthFlow.mjs', builders: [], extra: true, note: '2026-08-28 작성 / 2026-08-30 등록 — Paul Town 정원 성장 end-to-end 계약(84단언). 정원 성장축을 clearedCount 에서 gardenPoints 로 교체한 수정(7fa4641 계열)의 회귀 안전망인데, 작성 당시 registry 에 등록되지 않아 verify:all 에서 한 번도 실행되지 않고 있었다(야간 하네스 감사에서 발견 — 등록 101개 대비 미등록 test 스크립트 11개 중 하나). 네트워크 0 / Supabase 참조 0 이라 그대로 편입 가능했고, 편입 시점에 단독 실행으로 84단언 PASS 확인. 음수/NaN 포인트 클램프까지 포함한다. 2026-09-03 P1 레이어 계약 회귀 수정(useStudent.js가 attachment/worldProgress.js를 import하던 것을 되돌림) 이후 93단언(추가 9개는 애초 그 회귀를 잡아낸 "10) 보상 시스템 무영향" 절의 단언).' },
      { script: 'scripts/testGrowthPoints.mjs', builders: [], extra: false, note: '2026-09-03 P2 — 정원 성장 v2 보너스(src/utils/attachment/growthPoints.js) 순수 계약. 정원 성장축을 "학습한 단어 수(gardenPoints)"만 보던 기존 축에, 목표달성/오답복습 등 ledger 이벤트 기반 보너스를 옵션으로 얹는다(attachmentGardenGrowthV2 플래그, 기본 OFF — flag off는 기존 gardenPoints===gardenSet.size와 완전히 동일해야 함, 8절 "flag-off 동일성"이 이를 고정). epoch(2026-09-03) 이전 이벤트는 소급 적용하지 않고, 일일 상한(PER_DAY_CAPS)으로 보너스 인플레를 막는다. 정원 칸/월드 단계가 어떤 입력에도 역행하지 않음(7절 단조성)까지 포함 — 44단언, 네트워크 0. implementer 트랙이 registry 소유 범위에서 대신 등록(파일 내용 무수정).' },
  ...
  rewardSystem: {
    ...
    checks: [
      ...
      { script: 'scripts/testSessionRewardSummary.mjs', builders: [], extra: false, note: 'P1 "즉각적인 보상 피드백"(2026-09-03, verify:session-reward-summary와 동일 실행) — 학생이 세션 완료 앵커 4종(word-session-complete/writing-complete/exam-complete/daily-goal-complete) 중 하나를 달성한 순간, 그 세션에서 실제로 지급된 별/XP/정원 성장/연속일/다음 레벨을 모아 보여주는 하단 고정 카드(SessionRewardCard.jsx). 순수 요약 계산은 신규 zero-import 모듈 src/utils/rewardSummary.js(buildSessionRewardSummary/formatRewardLines, rewardEngine.js와 무관하게 독립 — LEVELS 임계값은 XP_EVENT_TABLE이 REWARD_STARS를 독립 복제하는 것과 같은 관례로 값만 복제, 드리프트는 이 테스트가 rewardEngine.starsToNextLevel과 직접 교차검증해서 막음)가 전담, useStudent.js는 grantLedgerReward의 기존 hasRewardEntry 조기반환(무변경) 통과 후에만 결과를 상태로 노출한다 — 새 dedup 없음, 재지급 로직 무변경. 신규 플래그 sessionRewardSummary(config/features.js) 기본 false — OFF면 기존 RewardToast만 그대로 동작(App.jsx가 studentData.sessionRewardSummary로만 카드를 조건부 마운트, useStudent.js가 플래그 OFF면 그 상태를 영원히 null로 유지). 검증: 순수 모듈 계약(zero-omission/레벨 경계 드리프트 가드 등 28단언) + SessionRewardCard SSR 렌더(요약 있음/summary=null/전부 0 3가지 케이스, z-40이 GiftReveal/HatCeremony보다 낮은지 포함 9단언) + 배선 정적 검사(플래그 기본값/기존 가드 재사용 순서/App.jsx 마운트 7단언) + 실제 rewardEngine.js export(hasRewardEntry/appendRewardEntry/buildRewardEntry)를 그대로 이어붙인 재지급 방지 시뮬레이션(같은 키 2회 호출 시 grantReward/grantXp/요약 생성 전부 정확히 1회, 5단언) = 42단언, 네트워크 0. 구현 전(모듈/컴포넌트 부재 + 배선 없음 상태) FAIL-first 실측: 12단언 FAIL(규칙 15) 후 구현해 전체 PASS 전환. 13개 필수 도메인 밖, 신규 보너스 커버리지.' },
      ...
      { script: 'scripts/testStreakV2Wiring.mjs', builders: [], extra: false, note: 'P10(2026-09-03) 등록 누락 발견분(commit f477709 패턴) — P6 Streak V2 배선(verify:streak-v2와 동일 실행) FAIL-first 계약 테스트. streakAdapter.js 순수 계약(연속/공백+freeze/리셋+best 보존/퀴즈 단독 인정일) + 레거시 calcStreak(useStudent.js) 시맨틱 무변경 소스 검사(이 세션은 useStudent.js 무수정) + Dashboard.jsx 정적 배선(streakV2 플래그) + StreakChip.jsx 자체 esbuild 번들 SSR 문자열 단언(flag-off/on). 구현 전 9 FAIL/8 PASS 실측(규칙 15, 파일 헤더 주석). 이미 구현·PASS 상태였고 package.json/registry 등록만 빠져 있었다(등록만 대신 수행, 파일 내용 무수정). 네트워크 0.' },
      { script: 'scripts/testAdminRewardSummary.mjs', builders: [], extra: false, note: 'P10(2026-09-03) 등록 누락 발견분 — P9 관리자 StudentDirectory 보상 요약 1줄(verify:admin-reward-summary와 동일 실행) 순수 로직(summarizeStudentRewards/formatAdminRewardLine, src/utils/adminRewardSummary.js) 계약. null/undefined/빈 행 안전 폴백, 레거시 행(rewardLedger/hatInventory 없는 v1.3~v1.4 레코드) 처리 포함. React/네트워크/Supabase 0 — plain Node import. 이미 구현·PASS 상태였고 등록만 빠져 있었다(등록만 대신 수행, 파일 내용 무수정).' },
      { script: 'scripts/testUnitCompleteReward.mjs', builders: ['unitComplete'], extra: false, note: 'P10(2026-09-03) 등록 누락 발견분 — P4 유닛 완료 보상(verify:unit-complete-reward와 동일 실행) FAIL-first 계약 테스트. recordUnitCompleted(useStudent.js, esbuild 번들+fakeReact 구동, flag unitCompleteReward OFF/ON·중복지급 방지·별개 unitId 별도 지급) + mergeProgressRecords 원장 병합(unit-complete 항목도 동일 idempotency_key union 규칙) + diffNewlyCompleted(unitCompletionDetector.js 순수 함수) 직접 검증 + useAttachment.js 배선 정적 검사. 이미 구현·PASS 상태였고 등록만 빠져 있었다(등록만 대신 수행, 파일 내용 무수정). 네트워크 0.' },
      { script: 'scripts/testMasteryReward.mjs', builders: ['unitComplete'], extra: false, note: 'P10(2026-09-03) 등록 누락 발견분 — P5 복습/숙달 보상 강화(verify:mastery-reward와 동일 실행) FAIL-first 계약 테스트. testUnitCompleteReward.mjs와 같은 번들(scripts/.tmp/useStudent.p4p5.bundle.mjs)과 기법 재사용(CLAUDE.md 규칙 3), flag masteryReward를 localStorage 프리시드 + 매 시나리오 cache-busted import로 제어. 이미 구현·PASS 상태였고 등록만 빠져 있었다(등록만 대신 수행, 파일 내용 무수정). 네트워크 0.' },
      { script: 'scripts/testRewardIntegrityV2.mjs', builders: ['unitComplete'], extra: false, note: 'P10(2026-09-03, docs/REWARD_LOOP_AUDIT_2026-09-03.md §14, verify:reward-integrity-v2와 동일 실행) — 운영자 지정 10개 실사용 무결성 시나리오 통합 스위트(개별 앵커/타입 계약은 재검증하지 않음, 그건 testRewardFlow.mjs/testRewardTypesV2.mjs/testUnitCompleteReward.mjs/testMasteryReward.mjs 소유 — 규칙 3). 1) 같은 세션 중복 제출(4앵커: word-session-complete/exam-complete/unit-complete/daily-goal-complete) → 각 1회분만. 2) refresh(재초기화 후 재로드) 추가 지급 0. 3) back/forward(연속 리마운트 2회) 추가 지급 0. 4) network retry — api/grant-xp.js에 동일 POST 10회 순차 → 원장 1건 + duplicate 플래그(esbuild+인메모리 가짜 Supabase, testRewardTypesV2.mjs와 동일 기법·별도 산출물명). 5) 두 기기 동시 요청 병합(mergeProgressRecords, 같은 idempotency_key 다른 id/created_at) → 1건, totalStars maxNum 규칙(중복 합산 아님) 문서화. 6) 완료된 unit 재진입 → diffNewlyCompleted 0개 + recordUnitCompleted 2차 호출 false. 7) 같은 단어 fail/pass 5일 반복 — word-mastered 평생 1건/review-session-bonus 1일1건 유지 + 서버 wrong-word-recovered 일일상한 60 실측(61번째 daily_cap_reached). 8) growthPoints/bonusPointsFromLedger/deriveAttachmentStats(gardenGrowthV2) 정원-ledger 일치(중복 키 미가산, epoch 이전 0, flag OFF 0). 9) 5개 신규 플래그(unitCompleteReward/masteryReward/streakV2/nextGoalsCard/attachmentGardenGrowthV2) 전부 OFF로 daily-goal 4/4 리플레이 델타 18 불변 + REWARD_STARS 7종/STREAK_BONUS/MISSION_BONUS_STARS를 파일 상단 인라인 동결 스냅샷(LEGACY_SNAPSHOT_84a36b8, 84a36b8 시점 값을 옮겨 적음)과 대조해 동결 확인 — CI shallow checkout(fetch-depth 1)에서 `git show 84a36b8:...` 파싱이 크래시하던 문제(2026-09-03 사후 QA, run 33729692487) 이후 git 의존 제거. 10) mergeProgressRecords no-reset — local이 뒤처진 경우/cloud가 뒤처진 경우 양방향 모두 totalStars/hatInventory/rewardLedger 길이가 절대 축소되지 않음. 53단언, 네트워크 0(4·7절 서버 시나리오는 esbuild 인메모리 번들, 실제 Supabase 미접촉). FAIL-first 실측(규칙 15): REWARD_DAILY_CAP[wrong-word-recovered]를 60→999로 임시 변경(즉시 원복) 후 재실행해 7절 4개 단언이 실제로 FAIL함을 확인 — 나머지는 이미 구현된 로직의 정직한 계약 동결(각 절 헤더 주석에 명시).' },
    ],
  },
