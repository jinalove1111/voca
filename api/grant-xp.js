...
export default async function handler(req, res) {
  ...
  if (req.body && req.body.ledger === 'reward') {
    ...
    // L2 안전 설계 결정 — 'unit-complete'는 exam-complete와 달리 L2 실재
    // 검증(서버가 관측 가능한 진실 테이블 대조)을 **의도적으로 두지 않는다**
    // (2026-09-03, P4). 근거:
    //   1) 유닛 완료를 서버가 검증하려면 word_status(단어별 학습 상태) 같은
    //      테이블을 대조해야 하는데, 그 테이블은 anon 클라이언트가 직접
    //      write하는 값이라(클라이언트가 자기 학습 상태를 스스로 기록하는
    //      구조) "검증"이라 부를 만한 신뢰 경계가 아니다 — 대조해도 클라이언트가
    //      이미 조작 가능한 값을 다시 읽는 것뿐이라 방어력이 없다(약한 필터를
    //      추가해 복잡도만 늘리는 것을 피함).
    //   2) 대신 안전은 세 겹으로 유한하다: unitId별 idempotency_key가 평생
    //      1회(같은 유닛으로 반복 지급 불가) + 하루 상한 2(REWARD_DAILY_CAP
    //      'unit-complete') + 유닛 개수 자체가 유한(교재당 정해진 유닛 수라
    //      결국 그 상한을 넘는 총 지급량 자체가 불가능). exam-complete의
    //      L2(entrance_test_results 대조)와 달리 unit-complete에는 이런
    //      "서버만 아는 진실" 테이블이 없으므로, 이 세 겹이 유일하고 충분한
    //      1차 방어선이다(운영자 승인 결정, docs/REWARD_LOOP_AUDIT_2026-09-03.md §14 P4).
    //   3) 'word-mastered'/'review-session-bonus'도 같은 이유로 L2가 없다 —
    //      각각 wordId 평생 1회+상한 60, 날짜 기반 하루 1건으로 이미 유한하다.
