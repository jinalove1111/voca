...
24. `supabase_v3_16_students_insert_lockdown.sql`(2026-08-06, P0 후속 — 학생 계정 중복 생성 차단) — `students` 테이블의 `INSERT` 권한만 `anon`/`authenticated`에서 회수(`revoke insert`). `SELECT`/`UPDATE`/`DELETE`는 전혀 건드리지 않음(v1.9 컬럼권한 그대로 유지). 신규 학생 생성은 이후 `api/admin-pin-actions.js`의 `create_student` 액션(관리자 인가 + `service_role`, 클라이언트 UUID 멱등키)이 유일한 경로가 된다. 의도적으로 `unique(name)` 등 이름 UNIQUE 제약은 추가하지 않음(v1.6부터 동명이인은 지원 사양 — 파일 헤더 주석 참고). **[미실행 — 운영자 실행 대기, 반드시 클라이언트 자기등록 경로 제거 배포(40~41차, 완료) 이후에 실행할 것 — 순서를 지키지 않아도 앱이 깨지지는 않고 구버전 화면의 자기등록만 42501로 실패함]**. 상세: `handoff.md` 2026-08-06(40~41차).

25. `supabase_v3_17_textbook_backfill_new_classes.sql`(2026-08-06, 42차 P0 후속 — 신설 반 교재 레이어 백필) — v3.1 백필(2026-07-22) 이후 새로 생성된 반 중 유닛은 있지만 `textbooks`/`class_textbooks`에 아직 없는 반만 대상으로 동일 패턴을 반복(`insert ... on conflict do nothing`, `update ... where textbook_id is null`). 파괴적 구문 0. **[선택사항 — SQL 미실행이어도 코드가 동일 백필을 자동 수행함(`ensureTextbookLayerBackfilled`, `AdminScreen.jsx` 마운트 시 fire-and-forget). 관리자 화면이 배포 이후 반복적으로 열려 이 자동 백필 경로로 사실상 적용된 것으로 확인됨(2026-08-07 라이브 관찰) — SQL 파일 자체를 SQL Editor에서 실행했다는 명시적 운영자 기록은 없음, 둘 다 실행해도 `on conflict do nothing`이라 중복 없음]**. 상세: `handoff.md` 2026-08-06(42차), `ARCHITECTURE.md` "학생 홈 반/교과서/유닛 정책" 절.

26. `supabase_v3_18_class_renames.sql`(2026-08-07, 운영자 지시 — 반 이름 변경 2건) — `classes.name` 컬럼만 `id`+현재 `name` 조건부 `UPDATE` 2줄(멱등, 롤백 역UPDATE 주석 동봉). `고1 6월 학평`→`MS Advanced Class`, `고1 능률 민병천`→`Presentation 6`. `classes.id`가 FK 기준이라 학생/유닛/단어/배정은 전혀 영향받지 않음(`textbooks.name`은 별개 컬럼이라 교과서 이름은 그대로 유지). anon 클라이언트의 직접 `UPDATE classes` 시도가 v3.11 락다운에 의해 조용히(에러 없이 0행) 차단되는 것이 이 SQL을 준비하게 된 배경. **[실행됨 — 운영자가 Supabase SQL Editor에서 실행 후 검증 SELECT로 2건 확인, `handoff.md` 2026-08-07(50차) "운영자 실행" 참고]**. 세 번째 지시("중등 예비반"→`Pre-Middle School`)는 대상 반이 `classes`에 존재하지 않아(신설 문제) 이 SQL이 다루지 않음 — 관리자 화면 "반 만들기"로 `Pre-Middle School`을 직접 생성하는 안내만 파일에 주석으로 동봉. 상세: `handoff.md` 2026-08-07(50~51차).

27. `supabase_v3_19_class_type_textbook_containers.sql`(2026-08-08, 54차 — 실반/교과서 컨테이너 판별 구조화) — 신규 컬럼 없음, 기존 `classes.class_type` 컬럼(값 `'regular'`/`'special'`만 쓰이던 상태)에 대해 교과서 컨테이너 반 6개(UUID 기준 조건부 UPDATE, `is distinct from` — 멱등)만 `'textbook'`로 값 갱신. 이름 기반 WHERE는 의도적으로 금지(`Presentation 6`과 `Presentation 6 -2026` 이름 충돌 미해결 상태라 위험). GRANT 불필요(`classes` SELECT는 기존에 이미 부여, `wordLibrary.js`가 이미 이 컬럼을 select에 포함). **[미실행 — 운영자 실행 대기]**. 실행 전에는 `src/utils/wordLibrary.js`의 신규 `classifyRealClassNames()`/`getRealClassNames()`가 `class_type='textbook'` 신호 부재를 감지해 기존 이름 allowlist(`['MS Advanced Class','Presentation 6','Pre-Middle School']`)로 폴백 — `StudentSelect.jsx`(PIN 만들기 반 목록)의 51차 동작과 100% 동일(회귀 없음). 실행 이후에는 이름 무관 구조 판별로 전환되어 반 개명/신설에 안전해진다. 상세: `handoff.md` 2026-08-08(54차).
...
### 2026-09-06 STAR SPENDING Phase 1 RLS(`supabase_v3_47_town_shop.sql`, 미실행 대기)

_추가: 2026-09-06(114차). 코드/SQL 전부 구현·작성 완료, 커밋 0, `townShopV1=false`, SQL 미실행 — 아래는 파일 원문 기준 계획된 RLS이며 아직 라이브에 적용되지 않았다. 상세: `handoff.md` 2026-09-06(114차)._

- `town_items` — `enable row level security` + 정책 1개(`"town_items anon read only"`, `for select using (true)`) + `grant select on table town_items to anon, authenticated`. INSERT/UPDATE/DELETE GRANT는 없음(가격/카탈로그 변경은 운영자 SQL Editor 또는 향후 관리자 API만).
...
## `backup_20260807_*` — A그룹(테스트 계정) 776건 백업 테이블 (`ops/`, 2026-08-07)

_추가: 2026-08-07(51차). 저장소 마이그레이션 관례(`supabase_*.sql`, 이 문서 상단)와 달리 이 테이블들은 `ops/account-cleanup-2026-08-07/`(신규 디렉터리, 아직 git 미추적) 아래 준비된 1회성 정리 스크립트가 만든다. **삭제 문은 준비만 됐고 실행되지 않았다** — `handoff.md` 2026-08-07(51차) "보류" 항목("A그룹 776건 삭제 보류 유지")과 정확히 일치._

- **대상**: 2026-08-07 전수 조사에서 확정한 776개 `students.id`(테스트 이름 패턴 검증 + PIN 없음 + 학습기록 0 + 당일 라이브 재검증 PASS). 실학생 109명/병합 검토 94명 계정은 이 목록에 없다(`ops/account-cleanup-2026-08-07/backup_create_and_verify_ONE_SHOT.sql` 헤더 주석).
- **백업 테이블 8개**(전부 `create table ... as select * from ... where id/student_id in (backup_20260807_students)`): `backup_20260807_students`(기준) + `backup_20260807_student_progress`/`student_daily_progress`/`word_status`/`spelling_review_queue`/`xp_ledger`/`student_class_assignments`/`entrance_test_results`. 6개 학습 관련 테이블은 대상이 테스트 계정이라 0행이 정상.
- **안전장치**: ①기존 `backup_20260807_*` 테이블이 하나라도 이미 있으면 즉시 `raise exception`으로 중단(덮어쓰기 금지) ②백업 후 `count(*) <> 776`이면 트랜잭션 전체 롤백(테이블 생성 자체가 취소됨) ③삭제 스크립트(`delete_safe_test_accounts_FINAL_NOT_EXECUTED.sql`, 파일명 자체가 미실행임을 명시)의 모든 `DELETE`가 `where id/student_id in (select id from backup_20260807_students)` 조건을 걸어서, 백업 테이블이 존재하지 않으면(=백업을 먼저 실행하지 않으면) 삭제 대상이 항상 0건이 되는 구조 — "백업 미실행 = 삭제도 불가능"을 스키마 레벨로 강제.
- **FK 삭제 순서**(삭제 스크립트에 명시, 자식→부모): `word_status → spelling_review_queue → student_daily_progress → student_progress → xp_ledger → student_class_assignments → entrance_test_results → students`.
- **접근 권한 — 확인 필요(정직 기록)**: 이 세 SQL 파일(`backup_create_and_verify_ONE_SHOT.sql`/`backup_create_and_verify_PASTE.sql`/`backup_safe_delete_candidates.sql`) 어디에도 `enable row level security`/`create policy`/`grant`/`revoke` 구문이 없다 — `CREATE TABLE ... AS SELECT`로 새로 만든 테이블에 RLS를 명시적으로 켰다거나 정책을 0개로 뒀다는 근거를 이 저장소 파일만으로는 확인할 수 없다. PostgREST 기본 동작상 anon/authenticated에 대한 명시적 `GRANT`가 없는 새 테이블은 조회가 막힐 가능성이 높지만(v1.9 이후 이 저장소의 관례), 이 문서는 "확인되지 않은 것을 확인됐다고 쓰지 않는다"(docs-maintainer 역할 원칙) 기준으로 접근권한 상태를 **미확인**으로 기록한다 — 실제 접근 가능 여부는 운영자가 Supabase 대시보드에서 `\d backup_20260807_students`(권한 목록) 또는 anon key로 직접 SELECT 시도해 재확인 권장.
- **정리 절차**(실행 시 참고): ①`backup_create_and_verify_ONE_SHOT.sql`(또는 `_PASTE.sql`) 먼저 실행해 776건 백업+검증 ②`delete_safe_test_accounts_FINAL_NOT_EXECUTED.sql`은 파일 상단에 "절대 실행 금지" 경고 3중 표시 — 운영자 명시 승인 없이는 실행 대상이 아님. 관련 파일: `C:\voca\ops\account-cleanup-2026-08-07\backup_create_and_verify_ONE_SHOT.sql`, `C:\voca\ops\account-cleanup-2026-08-07\backup_create_and_verify_PASTE.sql`, `C:\voca\ops\account-cleanup-2026-08-07\backup_safe_delete_candidates.sql`, `C:\voca\ops\account-cleanup-2026-08-07\delete_safe_test_accounts_FINAL_NOT_EXECUTED.sql`.

## `classes` 도메인 확정(2026-08-07) — "실제 반" vs "교과서 컨테이너 반"

_추가: 2026-08-07(49~51차). `classes` 테이블에는 원본 스키마 문서(위 "핵심 4테이블")가 구분하지 않는 두 종류의 행이 섞여 있다는 것이 49차 조사로 확정됐다 — v3.1 이전 "교과서=반" 구모델의 잔재._

- **실제 반(학생이 물리적으로 소속된 그룹)**: 2026-08-07 기준 확인된 것은 `MS Advanced Class`(舊 "고1 6월 학평", v3_18로 개명, 실행됨)와 `Presentation 6`(舊 "고1 능률 민병천", v3_18로 개명, 실행됨) 2개 + `Pre-Middle School`(운영자 지시 3번째 항목, **아직 미생성** — v3_18 SQL이 다루지 않고 관리자 화면 "반 만들기"로 생성 대기) 1개, 총 3개 슬롯.
- **교과서 컨테이너 반(콘텐츠 묶음만을 위해 존재, 학생이 "물리적으로 소속"된 반이 아님)**: `중2 능률 김기택`/`중2 천재 이상기`/`중2 YMB 박준원`/`중2 동아 윤정미`/`중1 동아 윤정미` 등 이름이 교과서/출판사/저자명을 그대로 딴 반들 — v3.1(다중 교재) 이전에는 "교과서를 바꾸려면 이 반으로 재등록"하는 구조였던 흔적. 49차 조사(`.ai-status/duplicate-account-audit-2026-08-07.md` 실측)에서 이런 교과서명 반에 **직접 소속(`students.class_id`)된 학생 59명**(능률 40명/천재 19명)이 확인됐다 — 이 학생들은 라벨(49차 PIN 만들기 화면 "[반]" 접두 배지)만으로는 완전히 해소되지 않고, 소속 반 이관(운영자 결정 대기, 별개 건)이 필요하다.
- **주의(미확정 수치)**: "교과서명 반 소속 학생 전체를 실제 반으로 이관해야 하는 총 인원"은 이 세션에서 재확인되지 않았다 — 위 59명은 43~49차 조사 시점 한정 수치(전체 교과서명 반이 아니라 일부에 대한 표본일 수 있음)이고, 더 큰 합계(예: 100명대)를 주장하는 근거는 `handoff.md`/`.ai-status/`/`ops/` 어디에서도 grep으로 확인되지 않았다 — 정확한 전체 인원이 필요하면 `scripts/dbIntegrityAudit.mjs` 계열로 새로 실측할 것(추측 금지).
- **실무 함의**: 관리자 화면에서 반 이름만 보고 "이게 진짜 반인가 교과서 묶음인가"를 헷갈리기 쉽다(`PROJECT_GUIDE.md` "헷갈리는 것" 목록에 항목 추가됨). PIN 만들기 로스터는 `students.class_id` 기준으로만 조회되므로(49차 확인, `getStudentsInClass`), 교과서명 반을 "반"으로 착각해 선택하면 실제 그 반 소속 학생만 나오고 다른 교과서를 배정받은 학생은 안 보인다 — 정상 동작이며 버그 아님.
