...
console.log('\n5. XP 이벤트 테이블 — 서버(api/grant-xp.js)가 신뢰하는 유일한 금액 원천')
{
  check('4개 addStars 트리거와 1:1 대응하는 6개 이벤트 타입(콤보 3종 포함)',
    Object.keys(XP_EVENT_TABLE).sort().join(',') ===
    ['duplicate-sticker-bonus', 'mission-bonus-4of4', 'mission-clear', 'spelling-combo-10', 'spelling-combo-3', 'spelling-combo-5'].sort().join(','))
  check('resolveXpAmount가 알려진 이벤트의 고정 금액을 반환', resolveXpAmount('mission-clear') === 3 && resolveXpAmount('mission-bonus-4of4') === 10 && resolveXpAmount('duplicate-sticker-bonus') === 20)
  ...
  check('모든 금액이 양의 정수(스키마 CHECK 제약과 일치)', Object.values(XP_EVENT_TABLE).every(v => Number.isInteger(v) && v > 0))
}
...
console.log('\n7. 입력 검증 헬퍼 (서버 api/grant-xp.js가 그대로 재사용)')
{
  ...
  check('sourceEventId — 정상 문자열 통과', isValidSourceEventId('mission-clear:abc-123') === true)
  check('isValidEventType이 resolveXpAmount와 일관됨', isValidEventType('mission-clear') === true && isValidEventType('nope') === false)
}
