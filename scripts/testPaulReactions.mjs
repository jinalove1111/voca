...
console.log('\n1. 모든 리액션이 필수 필드를 다 갖고 있고, image가 실제 로드된 값(빈 문자열 아님)임')
{
  ...
  // 2026-07-10: 이 테스트는 원래 "17개 real PNG" 시절 기준으로 짜여서
  // retry/brand/ponder/brand_love가 아직 없는 것으로 하드코딩돼 있었는데,
  // 실제로는 그 사이 21개로 늘어나 있었다(src/assets/paul/ 실물 파일
  // 21개, paulReactions.js도 이미 정확히 반영돼 있었음) — 소스가 아니라
  // 이 테스트만 오래돼서 냉간 실행 시 7개씩 오탐하던 것. 실제 파일
  // 개수에 맞춰 갱신.
  check('실제 개별 PNG가 있는 21개만 등록됨(나머지는 존재하지 않는 파일을 가리키지 않음)', PAUL_REACTIONS.length === 21)
}
...
console.log('\n2. getReactionById — id로 정확히 지정 가능(쓰기 4단계처럼 랜덤이면 안 되는 곳)')
{
  ...
  check('실제 PNG가 있는 retry도 정상 반환(2026-07-10: 17->21 확장분)', getReactionById('retry')?.id === 'retry')
}
...
console.log('\n6. pickReaction — 존재하지 않는 카테고리/캐릭터는 null(이모지로 대체하지 않고 조용히 실패)')
{
  ...
  // celebrate는 REQUESTED_BUT_MISSING에 여전히 남아있는(진짜 아직 없는) 캐릭터.
  check('아직 개별 PNG가 없는 캐릭터(celebrate)도 null', pickReaction('celebrate') === null)
}

console.log('\n7. 3개 카테고리(success/fail/study) 구조가 실제 src/assets/paul/ PNG 파일 21개와 정확히 일치')
{
  const byCategory = { success: 6, fail: 7, study: 8 }
...
console.log('\n8. 이전 호출부 하위호환 — pickReaction("encourage")/pickReaction("levelup")이 여전히 정상 동작')
{
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (!r || !['thinking', 'almost', 'one_more', 'retry'].includes(r.id)) { ok = false; break }
  }
  check('pickReaction("encourage")가 격려 계열 이미지 중 하나를 계속 정상 반환(retry 포함)', ok)
...
console.log('\n9. resolveReaction — type prop 하나로 id/카테고리/메시지별칭을 전부 처리')
{
  ...
  check('type="brand"(실제 PNG 있음, 2026-07-10: 17->21 확장분) -> study 카테고리 이미지', resolveReaction('brand')?.id === 'brand')
  check('type="celebrate"(진짜 아직 PNG 없음) -> null, 이모지로 대체 안 됨', resolveReaction('celebrate') === null)
}
