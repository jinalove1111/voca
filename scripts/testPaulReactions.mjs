...
console.log('\n7. 3개 카테고리(success/retry/etc) 구조가 실제 public/assets/paul/ PNG 파일 18개와 정확히 일치')
{
  const byFolder = { success: 6, retry: 6, etc: 6 }
  for (const [folder, count] of Object.entries(byFolder)) {
    check(`${folder} 카테고리 ${count}개`, PAUL_REACTIONS.filter(r => r.category === folder).length === count)
  }
  check('총 18개(실제 파일 개수와 일치, paul_brand.png 포함 — 이건 아직 파일이 없어 emoji로 보임)', PAUL_REACTIONS.length === 18)
}
...
console.log('\n9. resolveReaction — type prop 하나로 id/카테고리/메시지별칭을 전부 처리(요청사항 5)')
{
  ...
  check('type="retry"(카테고리)', resolveReaction('retry')?.category === 'retry')
  check('type="levelup"(정확한 id)', resolveReaction('levelup')?.id === 'levelup')
  check('type="fail"(메시지별칭) -> retry 카테고리의 실패계열 이미지', ['sad','cry','sorry'].includes(resolveReaction('fail')?.id))
...
console.log('\n11. 등록된 image 경로가 실제 public/assets/paul/ 파일과 존재 여부까지 일치하는지(요청사항 9 — "더 이상 이모지가 보이면 안 됨"의 실질적 검증)')
{
  const missing = PAUL_REACTIONS.filter(r => !existsSync(path.join(PUBLIC_DIR, r.image)))
  if (missing.length === 0) {
    check('18개 전부 실제 PNG 파일 존재 — emoji로 대체될 항목 없음', true)
  } else {
    console.log(`  (참고) 아직 파일 없어 emoji로 보이는 항목: ${missing.map(r => r.id).join(', ')}`)
    check('알려진 예외(brand)를 제외하면 나머지는 전부 실제 파일 존재', missing.every(r => r.id === 'brand'))
  }
}
