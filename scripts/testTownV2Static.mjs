...
// ── 3. src/assets/town/index.js — 드롭인 아트 12개 키만, V2 전용 에셋은 0개 ──
section('3. src/assets/town/index.js — 드롭인 아트 12개 키 외 무변경')
...
// 12번째 키(animals/owl)가 추가됐다 — 더 이상 빈 객체가 아니다
// (scripts/testTownUiStatic.mjs의 동일 계약과 함께 유지). 이 섹션의 핵심은
// "V2 전용 새 정적 에셋이 추가로 생기지 않았다"는 것이므로, 정확히 이 12개
...
check(
  'src/assets/town/index.js — TOWN_ASSETS가 정확히 batch1+batch2 12개 키만 포함(V2 전용 신규 에셋 0개)',
