...
//
// 2026-09-09 야간 QA — fetchAll이 PostgREST 서버 기본 상한(max-rows=1000)을
// 무시하고 `?limit=10000` 한 번만 쏴서, 1000행을 넘는 테이블(그날 words=2075)
// 은 조용히 잘려 있었다(scripts/lib/prodDataLoader.mjs selectAll,
// src/utils/wordLibrary.js selectAllRows와 동일한 이 저장소의 기존 "1000행
// 절단" 실사고 패턴 — 2026-08-12). 결과: "word_status.word_id가 존재하지
// 않는 단어를 가리킴 — 60건"이 words 뒷부분 1075개가 안 불려와서 생긴
// 오탐이었다(waste/type/charity 등 3건 표본 확인 결과 실제로는 존재). 이제
// fetchAllPaged()로 1000행 페이지네이션(order=id 고정 정렬)한다 —
// scripts/testDbIntegrityAuditPaging.mjs 회귀 참고.
//
...
// PostgREST 서버 기본 상한(max-rows)은 1000행이다 — limit을 그보다 크게
// 줘도 조용히 1000에서 잘린다. 페이지당 최대 pageSize(기본 1000)씩
// offset을 늘려가며 짧은 페이지(pageSize 미만)가 나올 때까지 반복한다.
// order를 고정해 페이지 경계에서 tie로 인한 행 누락/중복을 막는다
// (src/utils/wordLibrary.js selectAllRows 주석과 동일한 이유). 기본
// orderBy='id'지만, student_progress처럼 별도 id 컬럼 없이 student_id가
// PK인 테이블도 있어(supabase_v1_3_schema.sql:13 — v1.4가 id 컬럼을 추가
// 시도하지만 `create table if not exists`라 이미 있던 v1.3 테이블에는
// 적용되지 않는다, 라이브 실측 42703으로 확인) orderBy를 호출부가 override
// 할 수 있게 한다. fetchImpl도 주입 가능하게 해 오프라인(네트워크 0)으로
// 단위 테스트할 수 있게 한다(scripts/testDbIntegrityAuditPaging.mjs).
export async function fetchAllPaged(baseUrl, headers, table, select, { pageSize = 1000, fetchImpl = fetch, orderBy = 'id' } = {}) {
  const out = []
  for (let offset = 0; ; offset += pageSize) {
    const url = `${baseUrl}/rest/v1/${table}?select=${select}&order=${orderBy}&limit=${pageSize}&offset=${offset}`
    const res = await fetchImpl(url, { headers, signal: AbortSignal.timeout(20000) })
    if (!res.ok) {
      const bodyText = (await res.text()).slice(0, 200)
      throw new Error(`${table}: HTTP ${res.status} ${bodyText}`)
    }
...
async function main() {
  ...
  async function fetchAll(table, select, { quiet = false, orderBy = 'id' } = {}) {
    try {
      return await fetchAllPaged(BASE, H, table, select, { orderBy })
    } catch (err) {
      if (!quiet) console.log(`  [조회 불가] ${err.message}`)
      return null
    }
  }
  ...
  // student_progress는 별도 id 컬럼이 없다(student_id가 PK) — order=id로
  // 조회하면 42703(column does not exist)로 실패해 전체 5)절 검사가 조용히
  // 스킵된다(2026-09-09 라이브 실측으로 발견). student_id로 정렬(unique라
  // 결정적 페이지 경계 보장).
  const progress = await fetchAll('student_progress', 'student_id,updated_at', { orderBy: 'student_id' })
