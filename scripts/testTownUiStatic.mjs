...
// 10번째 키(red-post-box), 11번째 키(animals/cat)가 추가됐다(더 이상 빈
// 객체가 아님) — 소스 텍스트에서 `TOWN_ASSETS = { ... }` 객체 리터럴
...
const STILL_EMOJI_ONLY_ASSET_KEYS = [
  ...
  'animals/puppy', 'animals/owl',
]
check(
  'src/assets/town/index.js — TOWN_ASSETS가 정확히 11개 드롭인 키를 포함',
...
check(
  'src/assets/town/index.js — TOWN_ASSETS에 나머지 12개 카탈로그 키는 여전히 없음(이모지 폴백 유지)',
