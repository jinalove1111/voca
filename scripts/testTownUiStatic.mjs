...
// 10번째 키(red-post-box), 11번째 키(animals/cat), 12번째 키(animals/owl)가
// 추가됐다(더 이상 빈 객체가 아님) — 소스 텍스트에서 `TOWN_ASSETS = { ... }`
// 객체 리터럴 블록만 잘라내 그 안의 키 목록을 정규식으로 추출한다(plain
// Node는 .webp 정적 import를 해석 못 하므로 실제 import 대신 소스 텍스트
// 검사로 계약을 고정한다).
...
const EXPECTED_TOWN_ASSET_KEYS = [
  'buildings/my-house', 'buildings/british-cottage', 'buildings/book-shop', 'decorations/red-post-box', 'animals/cat', 'animals/owl', 'nature/tree',
...
check(
  'src/assets/town/index.js — TOWN_ASSETS가 정확히 12개 드롭인 키를 포함',
...
check(
  'src/assets/town/index.js — TOWN_ASSETS에 나머지 11개 카탈로그 키는 여전히 없음(이모지 폴백 유지)',
