...
// ── 4. 마을 이미지 에셋 — 정확히 Batch 1+2+3 17개만 번들됨(그 외 0개) ────
...
// (decorations/town-sign)이 추가되어 17개로 갱신.
//
// ⚠ red-post-box·cat·owl·puppy는 실측(빌드 산출물 직접 확인)상 나머지
// 13개와 다르게 동작한다 — 다른 발견 사항을 그대로 보고: red-post-box.webp
...
// data:image/webp;base64 URL로 직접 인라인된다(나머지 13개는 전부
...
// english-school.webp(24442바이트)·town-sign.webp(5464바이트)도
// 동일하게 물리 파일로 방출된다. 이는 Vite의 표준 동작이며 회귀가
// 아니다 — 그래서 "물리 파일 13개" 계약과 "인라인 4개" 계약을 분리해서
// 검사한다.
section('4. 마을 이미지 에셋 — Batch 1+2+3 17개만 번들, 그 외 0개')
...
// 물리 파일로 방출될 것으로 기대되는 13개(red-post-box/cat/owl/puppy 제외 — 위 설명 참고).
const EXPECTED_BATCH1_IMAGE_BASENAMES = [
  'my-house', 'british-cottage', 'book-shop', 'cafe', 'bridge', 'english-school', 'town-sign', 'tree',
...
check(
  '마을 이미지 중 Batch 1+2+3 물리 파일 13개(집/코티지/책방/카페/다리/영어학교/표지판/나무/정원 5단계; red-post-box/cat/owl/puppy는 인라인이라 물리 파일 목록에서 제외) 외의 예상치 못한 파일이 dist/assets에 없음',
...
check(
  'Batch 1+2+3 물리 파일 13개 asset_key가 전부 dist/assets에 정확히 존재(webp 1개씩)',
