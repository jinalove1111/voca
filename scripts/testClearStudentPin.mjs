...
// supabase_v1_9_security_rls.sql 적용 후에는 anon 키가 pin 컬럼을 SELECT할
// 수 없다(의도된 차단 — 이 스크립트가 하던 "직접 읽기"가 바로 막으려던
// 공격 경로다). 그 경우 서버 API(student-pin-status, service role)의
// 부울 응답으로 동등한 검증을 한다 — SQL 적용 전/후 어느 상태에서든
// 이 스크립트는 그대로 통과해야 한다.
const fetchRow = async (id) => {
  ...
  const res = await call('student-pin-status', { studentIds: [id] })
  const s = res.body.results?.[0]
  if (!s) return null
  return {
    id: s.id,
    pin_hash: s.hasPinHash ? '(hidden by v1_9 RLS — hasPinHash=true)' : null,
    pin_setup_allowed: s.pinSetupAllowed,
    pin_fail_count: s.locked ? NaN : 0,
    pin_locked_until: s.locked ? '(locked)' : null,
  }
}
...
{
  const noAuth = await call('set-student-pin', { studentId: qa.id })
  check('[보안/P7 후속] adminPin 없는 무작위 재설정 → not_authorized 거부', noAuth.body.ok === false && noAuth.body.reason === 'not_authorized', noAuth.body)
  const res = await call('set-student-pin', { studentId: qa.id, adminPin: ADMIN_PIN })
