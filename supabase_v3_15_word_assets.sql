...
--   · 규칙 기반 백필(AI 미사용) — 기존 words의 example_text/memory_tip을
--     그대로 word_assets에 복사해 고유 단어 행을 만든다(정확한 예상
--     행 수는 바로 아래 "수치 갱신" 문단과 하단 검증 쿼리 (b-0) 참고 —
--     교사가 단어를 계속 추가하므로 이 파일 작성 시점 숫자를 고정값으로
--     믿지 말 것).
--
-- 왜 words를 안 건드리는가(실측 근거):
--   · words 테이블 실측(2026-08-04, 오케스트레이터 제공): 총 691행 / 고유
--     단어(대소문자·공백 무시) 599개 / 중복 13.3%(최다 4회 등장). 즉 같은
--     단어가 여러 유닛에 최대 4번 중복 등록돼 있다.
--   · **수치 갱신(2026-08-05 재측정)**: 그 사이 교사가 단어를 계속
--     추가해 총 811행 / 고유 695개로 늘었다. 이 숫자도 실행 시점에는
--     또 달라져 있을 것이다 — "정확한 기대값"은 이 파일의 숫자가 아니라
--     실행 직전 아래 (b-0) 쿼리로 직접 구할 것(그래야 이 파일이 다시는
--     낡지 않는다). 아래 "왜 words를 안 건드리는가" 결론(스키마 무변경/
--     백필 불필요/완전 가역)은 행 수와 무관하게 그대로 유효.
--   · words 스키마를 전혀 바꾸지 않으면 기존 학생·반·숙제·채점 경로에
--     구조적으로 영향이 0이고, 기존 words 행에 대한 백필도 불필요하며
--     (신규 word_assets에는 고유 단어 수만큼만 채우면 됨), 롤백은
--     "클라이언트가 word_assets를 조회하지 않게 하는 것" 하나로 완전
--     가역이다.
--   · word_key(정규화 텍스트) 단위로 묶으면 유닛 컨텍스트가 사라진다 —
--     예: 2026-08-04 당시 표본(총 691행 기준) 중복 단어 81종을 조사한
--     결과 41종은 유닛마다 뜻이 다르다(temperature = 체온/기온 등, 동철
--     이의 아님·같은 표기·다른 문맥 뜻). 이 81/41이라는 구체적 개수는
--     그 표본 시점 값이라 words가 늘어난 지금은 그대로 재현되지 않겠지만,
--     "같은 표기·다른 문맥 뜻이 실제로 존재한다"는 설계 근거 자체는
--     여전히 유효하다 — 그래서 이 테이블은 "채점 진실"이 아니라 "보조
--     콘텐츠 자산"으로만 설계됐다 — 아래 meaning_primary 컬럼 주석 참고.
--
...
-- 그대로 복사). words → 고유 word_key로 중복 제거(distinct on). 작성
-- 시점 실측은 691행/고유 599개(2026-08-04) → 811행/고유 695개(2026-08-05
-- 재측정, 그 사이 교사가 단어를 추가함) — 이 수는 단어가 추가될 때마다
-- 계속 늘어나므로, 정확한 기대 백필 행 수는 아래 (b-0) 쿼리로 실행 직전에
-- 직접 구할 것(이 주석의 숫자를 고정값으로 믿지 말 것).
...
-- (b-0) 실행 직전 기대값 확인(반드시 (b)보다 먼저 돌려볼 것 — words가
--     계속 늘어나므로 이 파일에 적힌 691/811 같은 숫자가 아니라 이
--     쿼리의 "지금" 결과가 기대값이다):
--   select count(distinct lower(btrim(word))) from words where word is not null and btrim(word) <> '';
-- (b) 백필 결과 행수 확인 — 위 (b-0) 결과와 정확히 일치해야 함:
