...
_최종 갱신: 2026-09-16 (143차 — Paul Town 월드 레이아웃 재설계(디자인
전용): 운영자가 실기기에서 V2(달러/잔액/소유/배치/이동/영속/격자 제거)
기능은 정상 동작하나 시각적으로 "보드 위 오브젝트"처럼 보여 NOT
APPROVED로 판정, 운영자 제공 참조 이미지(영국 스토리북 마을그림.png)를
아트 디렉션 기준으로 채택해 (1) 현재 씬이 보드로 읽히는 원인 6개 감사,
(2) 세로 스택 6-district 월드 구조 확정, (3) 인터랙티브 와이어프레임
(360/390/430px, 3라운드 수정 + 스크린샷 10장) 제작, (4) 최종 아트워크
스펙(P0/P1/P2, 캔버스/시점/조명/팔레트/QA 게이트) 작성, (5) P0 16파일
아트 생성 프롬프트 핸드오프 작성까지 완료. 전부 문서·와이어프레임
산출물이며 Production DB/SQL/코드/플래그/구매/merge/deploy는 전부
0건(REVIEW ONLY, 미merge·미push). 142차 이하 보존)_
...
_전부 워킹트리(uncommitted), 브랜치 `feat/paul-dollar-v1`. 커밋 0, push 0,
SQL 실행 0. 학습 자체는 멈출 필요가 없다(아래 "배포 순서" 참고)._

### 배경 — 왜 별을 소비 화폐에서 분리하는가

- v3_47(Town Shop V1)까지는 상점 구매가 `star_purchases`를 통해 **별
  (stars)을 직접 차감**하는 구조였다 — `purchase_town_item()`이
  `earned − spent`로 잔액을 계산하고 구매 시 `stars_spent`만큼 실제로
  줄였다.
- 운영자가 "별은 학습 성취를 나타내는 누적 지표로 절대 줄어들면 안 되고,
  상점 소비는 별개의 화폐(폴달러)로 분리해야 한다"고 지시(2026-09-08
  승인, 아래 "승인" 절)했다 — 학생이 상점에서 아이템을 사면 지금까지 쌓은
  "실력의 증거"인 별 배지 숫자가 줄어드는 것이 성취 지표로서 부적절하다는
  판단.

### 승인(운영자, 2026-09-08)

- **규칙 A**: 컷오버 시점부터 서버가 지급하는 별 1개 = 폴달러 $1로 전환
  (`dollars_per_star = 1`, 전 12종 보상 타입 동일 배율).
- **폴달러는 $0에서 시작** — 컷오버 시점에 레거시 별(그동안 쌓인
  `reward_totals.earned_stars`)을 폴달러로 소급 환산해 지급하지 않는다.
  모든 학생이 이 SQL 실행 이후 새로 버는 별부터 폴달러가 붙기 시작한다.
- **램프 가격 $60 유지** — `town_items.price` 컬럼 값(60)은 무변경, 화폐
  단위만 별에서 폴달러로 바뀐다(`price_currency` 컬럼 신설, 기본값
  `'dollars'`).

### 설계 — 두 화폐, 두 개의 진실 원천(`supabase_v3_49_paul_dollar.sql`)

- ⭐ **별(stars)** = 서버 권위 누적 업적. 단일 진실 원천은 여전히
  `reward_totals`(v3_36, `reward_ledger.stars_delta`의 합) —
  `student_progress.total_stars`(클라이언트 표시 캐시)는 이 파일도 절대
  읽지 않는다. **별은 이 SQL 이후 절대 감소하지 않는다** — 구매가 더
  이상 별을 차감하지 않으므로(v3_47의 별 기반 차감 RPC 본문을 이 파일이
  완전히 대체), 별은 오직 `reward_ledger`에 새 행이 쌓일 때만(양수만,
  기존 `check(stars_delta >= 0)`) 늘어난다.
- 💵 **폴달러(Paul Dollar)** = 상점 전용 화폐. 단일 진실 원천은 신규
  `dollar_ledger.dollars_delta`의 합(= `dollar_balances` 파생 뷰) — 저장된
  합계 컬럼 없음(`reward_totals`/`xp_totals`와 동일 판단, 사본-원본
  불일치 버그 자체가 존재할 수 없게).
- `dollar_rules`(신규, PK `reward_type`) — 어떤 보상이 폴달러로도
  전환되는지의 화이트리스트. `src/utils/rewardEngine.js`
  `REWARD_STARS`/`REWARD_SOURCE_RULES`에 실제 존재하는 12개 학습 보상
  타입만 시드(`word-session-complete`/`writing-complete`/`exam-complete`/
  `wrong-word-recovered`/`daily-goal-complete`/`streak-bonus`/
  `pronunciation`/`mission-clear`/`daily-mission-bonus`/`spelling-combo`/
  `sticker-duplicate`/`matchgame`, 전부 `dollars_per_star=1`) —
  **`legacy-baseline`은 의도적으로 시드하지 않는다**(레거시 이관 행이
  폴달러로 다시 환산되면 "레거시 별은 환산하지 않는다"는 승인된 설계가
  깨진다). `on conflict do nothing`이라 재실행해도 운영자가 이후 조정한
  배율/on-off를 덮어쓰지 않음 — **배율/on-off 조정은 코드 배포 없이
  SQL `UPDATE dollar_rules SET ...` 한 줄로 가능**하도록 설계.
- `trg_reward_ledger_to_dollars`(AFTER INSERT 트리거, `reward_ledger`) +
  `fn_reward_ledger_to_dollars()`(트리거 함수) — `stars_delta > 0`이고
  `dollar_rules`에 활성 규칙이 있으면 `stars_delta × dollars_per_star`를
  `dollar_ledger`에 자동 삽입(`idempotency_key = reward_ledger.
  idempotency_key || ':dollar'`). 함수 전체를 `EXCEPTION WHEN OTHERS`로
  감싸 폴달러 계산이 어떤 이유로든 실패해도 **원 별 지급(INSERT) 자체는
  절대 막지 않는다**(CLAUDE.md 규칙 1) — 실패 시 `RAISE WARNING`만 남김.
- `town_purchases`(신규 테이블, `star_purchases`와 완전히 분리) — 폴달러
  구매 이력 전용. 원안(`star_purchases`에 `currency`/`price_paid` 컬럼을
  얹는 것)을 기각한 이유: `star_purchases.stars_spent`가 이미
  `not null check(stars_spent > 0)`으로 고정돼 있어 폴달러 구매(별을 전혀
  안 씀)를 억지로 넣으려면 그 컬럼에 오해를 부르는 값을 채워야 하고, 이
  제약을 완화하려면 이 저장소의 destructive-SQL 게이트(제약 제거 구문
  전반 차단, CLAUDE.md 규칙 18)에 막힌다. 그래서 `star_purchases`는
  v3_47이 남긴 모습 그대로 완전히 보존(불변 감사 기록, Paul QA
  `shop-lamp` 1행 포함 — 이 파일에 그 테이블을 향한 쓰기 구문이
  0건)하고, "이미 가진 아이템인가"는 `star_purchases` ∪ `town_purchases`
  로만 판정한다(`get_town_shop_state()`).
- `get_town_shop_state(uuid)` — 반환 `(stars_earned, dollars_available,
  dollars_earned, dollars_spent, owned_item_ids)`. `stars_earned`은
  `reward_totals`에서만, `owned_item_ids`는 두 구매 테이블의 UNION.
- `purchase_town_item(uuid, text)` — 폴달러 전용. `price_currency ≠
  'dollars'`면 `item_not_purchasable`. 학생별 advisory lock → 잔액
  (`dollar_balances`) 계산 → 이미 소유(두 테이블 어느 쪽이든) →
  `insufficient` → `town_purchases` INSERT + `dollar_ledger` 음수 INSERT를
  같은 트랜잭션(함수 본문)에서 원자적으로 실행. `unique_violation` →
  `already_owned`로 안전 수렴(advisory lock이 순차 처리를 보장하지만
  이중 방어).
- `town_items.price_currency` 컬럼 추가(기본값 `'dollars'`, `check in
  ('stars','dollars')`) — 기존 컬럼 0개 변경, `price`(60) 무변경.

### 서버 (`api/grant-xp.js` diff)

- `get_town_shop_state`/`purchase_town_item` action 2개는 그대로 유지(새
  action 없음, 새 Vercel 함수 파일 없음, 12/12 한도 유지) — 응답 바디
  키만 새 계약으로 교체:
  - `get_town_shop_state` → `{ ok, starsEarned, dollarsAvailable,
    dollarsEarned, dollarsSpent, owned, items }`(`items[].priceCurrency`
    추가). `price_currency` 컬럼이 아직 배포 전(v3_49 미실행) 환경에서는
    `town_items` select 자체가 에러가 나므로, 그 경우에만 컬럼 없이
    재조회하고 `priceCurrency`를 `'dollars'`로 고정(규칙 9 폴백).
  - `purchase_town_item` → `{ ok, reason, dollarsSpent, balanceAfter,
    duplicate }`(레거시 `starsSpent`/`balanceAfter`(별 잔액) 필드는 더 이상
    응답에 없음).
  - 두 action 모두 `req.body.studentId`/가격/잔액을 여전히 참조하지
    않는다(세션 토큰 `sid`로만 학생 식별, TOCTOU 여지 없음 — 기존
    114차와 동일 신뢰 경계, 이번 세션에서 변경 없음).

### 클라이언트

- `src/utils/townShop.js` — `shopItemState`가 `dollarsAvailable` 기준으로
  판정(레거시 `available` 파라미터명도 별칭으로 계속 허용).
  `applyPurchaseResult`는 `dollars.available`만 갱신하고 **`starsEarned`는
  이 함수가 절대 건드리지 않는다**. `normalizeShopState`는 신규 계약
  (`starsEarned`/`dollarsAvailable`/`dollarsEarned`/`dollarsSpent`)을
  정규화하고, 레거시 응답(`available`만 있고 `dollarsAvailable` 없음)도
  안전하게 `dollars.available`로 흡수(과도기 방어). `formatDollars(n)`
  신규 export(`$60`처럼 정수+통화기호, 음수/NaN 0 클램프).
- `src/components/Dashboard.jsx` — `wallet` prop(`App.jsx:733`,
  `townShopEnabled && townShop.state`일 때만 `{starsEarned,
  dollarsAvailable}` 객체, 아니면 `null`). `wallet !== null`이면 ⭐ 배지가
  `starsDisplay` 대신 `wallet.starsEarned`(누적 성취, title="누적
  별(성취)")를 보여주고, 바로 옆에 💵 초록 배지(`formatDollars(wallet.
  dollarsAvailable)`, title="사용 가능한 Paul Dollar")가 추가된다.
  `wallet === null`(기본, 플래그 OFF)이면 이 분기는 전혀 타지 않아 기존
  배지와 완전히 동일(byte 단위 무회귀).
- `src/components/PaulTown.jsx` — 상점 줄이 💵 잔액 배지(`shop.state`
  로드 후에만 표시) + `💡 책상 램프 — $60`(아이템 줄, `shop.state` 로딩
  전에도 항상 보임) + 구매/부족(`💵 $N 더 필요`)/구매 중/보유 중 버튼
  4분기. **⭐는 이 상점 UI 어디에도 등장하지 않는다**(별은 상점에서
  소비되지 않는다는 설계를 화면에서도 지킴).
- **새 클라이언트 기능 플래그를 추가하지 않았다** — 화폐 자체는 서버가
  결정(응답 계약 교체)하고, `townShopV1`(`src/config/features.js:73`,
  여전히 `false`)이 화면 노출 여부의 유일한 게이트로 남는다.

### 테스트

- `scripts/testPaulDollarSql.mjs`(신규) — `supabase_v3_49_paul_dollar.sql`
  +`_ROLLBACK.sql` 정적 단언(`dollar_rules` 12종 시드/`legacy-baseline`
  제외, `dollar_ledger` unique idempotency_key + RLS 정책0·GRANT0,
  `dollar_balances` 뷰, 트리거 exception 래핑, `town_items.price_currency`
  기본 dollars + `price` 60 무변경, RPC 2개 재정의 계약, 권한, 롤백 WHERE절)
  + 순수 JS 인메모리 시뮬레이션(지급→달러 동액/legacy-baseline $0/별 많고
  달러 부족→insufficient/구매 후 별 불변·달러 −60/재시도·더블
  already_owned/동시 구매 직렬화/Paul QA stars 행 보존). 정적 `check()`
  호출 165개(grep 실측 — 실제 실행 결과는 리드가 최종 확인). 네트워크 0,
  SQL 실행 0.
- `scripts/testTownShop.mjs`(확장, 신규 파일 아님) — 75→101단언, 2재화
  분리 반영(`dollarsAvailable` 기준 판정/`applyPurchaseResult`가
  `starsEarned` 불변 단언/신규 `normalizeShopState` 계약).
- `scripts/testTownShopServer.mjs`(확장, 신규 파일 아님) — 47→65단언,
  응답 키 교체 + `price_currency` 컬럼 부재 폴백 시나리오 반영.
- `tests/harness/registry.mjs`에 `testPaulDollarSql.mjs` 등록
  (`extra:false`, 필수 게이팅).
- 신규 `package.json` 스크립트: `verify:paul-dollar` =
  `testPaulDollarSql.mjs && testTownShopSql.mjs && testTownShopServer.mjs
  && testTownShop.mjs`.
- 회귀 확인(무회귀): `verify:stars`/`verify:reward`/`verify:reward-server`/
  `verify:double-events`/`verify:persistence`/`verify:paul-town-
  progression`/`verify:cutover`/`verify:legacy-reward`. Release Gate 최종
  판정: **리드가 최종 확인**.

### 배포 순서(v3_49)

1. 코드 배포(플래그 `townShopV1=false` 그대로).
2. `node scripts/preflightTownShop.mjs --expect post-v3_49` — "아직 미적용"
   확인(신규 모드, `dollar_rules`/`dollar_ledger`/`dollar_balances`/
   `town_purchases` 네 객체가 401/42501=미배포로 나와야 정상).
3. 운영자가 Supabase SQL Editor에서 `supabase_v3_49_paul_dollar.sql` 실행
   (v3_47/v3_48 실행 확인 후 1회, BEGIN/COMMIT로 부분 적용 방지).
4. Post-verify(운영자 SELECT): `dollar_rules` 12행 / 트리거
   `trg_reward_ledger_to_dollars` 1행(`reward_ledger` 대상) / 함수 3개
   (`fn_reward_ledger_to_dollars`/`purchase_town_item`/
   `get_town_shop_state`) 전부 `prosecdef=true` / `dollar_ledger` 0행
   (소급 백필 없음) / `town_purchases` 0행 / `star_purchases` 1행 불변
   (Paul QA `shop-lamp`).
5. QA(Paul) 계정 실측: 별을 벌어 폴달러가 함께 쌓이는지 → 구매(⭐ 불변 /
   💵 −60) → 재로그인 영속성 확인.
6. 전부 통과 후에만 `townShopV1` ON 결정(별도 승인 사안, 이번 세션
   범위 밖).

### 롤백

`supabase_v3_49_paul_dollar_ROLLBACK.sql` — 트리거+트리거 함수 제거, RPC
2개를 v3_47의 별 기반 본문 그대로 원복(시그니처 동일), `dollar_balances`
뷰 제거, `dollar_ledger`/`town_purchases`에서 이 마이그레이션이 만든 행만
정확히 WHERE 삭제(CLAUDE.md 규칙 18, 무조건 DELETE 금지). 테이블 구조/
`dollar_rules` 시드 행/`star_purchases`(전체)는 전부 보존. **⚠️ 롤백을
실행하면 구매 시 별이 다시 차감되는 v3_47 예전 동작이 되살아난다** —
`townShopV1`이 이미 학생에게 ON으로 배포돼 있다면 롤백 전에 반드시 플래그를
OFF로 내릴 것(SQL 파일 자체는 플래그를 건드릴 수 없음).

### 잔여 / 알려진 한계

- 폴달러는 컷오버 이후부터만 쌓인다 — Paul QA의 레거시 램프 소유는
  `star_purchases`에 그대로 남아 "보유"로 계속 인정된다(재구매 없음).
- 첫 로그인 시 상점 상태 조회가 reconcile(115차 CUTOVER RACE 정산)보다
  먼저 로드되면, 다음 마운트 전까지 ⭐ 표시가 잠깐 실제보다 낮게 보일 수
  있는 레이스가 있다(P2, 신규 이슈 아님 — 115차가 이미 다룬 종류의 잔여
  리스크와 동일 계열, 데이터 손실 없음·다음 마운트에서 자연 교정).

### 다음 세션 주의

- **별을 다시 소비 재화로 쓰지 말 것** — `star_purchases`/별 차감 로직을
  어떤 이유로든 부활시키지 않는다(이 세션의 핵심 설계 결정을 되돌리는
  방향의 "개선"은 금지, CLAUDE.md 규칙 3과 같은 정신).
- `dollar_rules`의 배율(`dollars_per_star`)/`active` 값 조정은 SQL
  `UPDATE dollar_rules SET ...`만으로 가능하다 — 코드 배포가 필요하지
  않다(운영자가 직접 조정 가능하도록 설계된 부분, 새로 코드를 추가하지
  말 것).
- **`legacy-baseline`을 `dollar_rules`에 절대 추가하지 말 것** — 추가하는
  순간 레거시 이관 행(v1/v2)이 폴달러로 소급 환산되어 "컷오버 이전 별은
  환산하지 않는다"는 승인된 설계가 깨진다.
...
확인**. 이어서 (a) 나머지 피해 학생 전원 해결 여부 검증, (b) 관리자 분모
집계 버그를 수정했다.

### (a) 검증 — 배포본 함수로 전수 재검증

- 이전 누락 8명(Jinaa·황성연·김가윤 / 권교빈·김태율·황다은·전하은·김보민)
  **전원 조회 범위에 포함** ✅
- 실제 학생 41명 전수: "학습 중인 교재 반이 조회 범위에 없는 학생" **0명**
  (수정 전 8명)
- 오늘 시험(고1 능률 민병천 active) 실제 노출 **8명 전원 빨간 배너** —
  최초 0명 → 90차 4명 → 91차 8명
- 참고: 김보민은 `고1 6월 학평` 학습 중이지만 `고1 능률 민병천` 교재도
  배정돼 있어 오늘 시험이 보인다(배정이 실제와 맞는지는 운영자 확인 사항 —
  데이터는 건드리지 않음). 또한 한 학생에게 서로 다른 반의 active 시험이
  동시에 생기면 먼저 생성된 것 하나만 보인다(현재 해당 없음).

### (b) 관리자 "반 전체 N명"이 양방향으로 틀렸던 이유

`EntranceTestAdmin.jsx`의 분모만 `getStudentsInClass`(= `students.class_id`,
v1.x 정의)에 남아 있었다. 학생 화면은 이미 `class_id ∪ SCA ∪ 교재 소유 반`
으로 판정하는데 관리자만 옛 축이라:

- **교재 컨테이너 반 → 0명**(시험을 실제로 여는 반인데 분모가 0)
- **사람 반 → 과다 집계**: MS Advanced Class **132명**(실제 학생 15명,
  나머지 117개가 아카이브/중복 계정)

즉 "제출 6명 / 반 전체 132명"처럼 이미 무의미한 숫자였다.

### 수정 (`3b7e313`)

`wordLibrary.js`에 **관리자 전용** `fetchEntranceRosterForClass(classId)` 추가:

```
students.class_id = X ∪ SCA.class_id = X ∪ SCA.textbook_id = (X 소유 교재)
  − 아카이브/중복/QA 계정(isArchivedOrFixtureStudentName)
```

학생 화면 eligibility와 **같은 집합**이다. `getStudentsInClass`는 일부러
그대로 뒀다 — 반 관리 학생 목록/배정 이력/대시보드 매핑 등 기존 4개
호출부는 "이 반에 직접 소속된 학생"이라는 원래 의미를 계속 쓴다.
...
| 중1 동아 윤정미 | 0 | **3** |
| 중2 천재 이상기 | 0 | **1** |
| MS Advanced Class | 132 | **15** |
| Presentation 6 -2026 | 60 | **15** |
| Presentation 6 | 11 | **12** |
| Pre-Middle School | 9 | 9 |

**MS Advanced 132→15 명단 검증**: 빠진 117명이 **전원** `_DUP`/`_INACTIVE`/
QA 계정임을 프로그램으로 확인, 실제 학생 손실 **0명**, 신규 유입 0명.
전 반 합계로도 잔존 아카이브 0건 / 사라진 실제 학생 0명.

시험 생성·채점·제출·랭킹 로직 무변경. 관리자 화면에서만 반 변경 시 조회
1건(5초 폴링에 얹지 않음) — 학생 클라이언트 부하 0. **DB 변경 0.**

### 테스트 하네스 자체 결함 1건 발견·수정

새 검증 섹션이 처음에 FAIL했는데, 원인은 프로덕션 코드가 아니라 테스트의
`restGet`이 PostgREST 기본 1000행 상한에 잘려(students 1157행) 기대 집합을
잘못 계산한 것이었다. `restGetAll` 페이징으로 수정 — 앱의
`refreshStudents`는 이미 페이징돼 있어(38차 `8e15ff7`) 프로덕션 영향은
없었다. **"테스트가 틀렸는지 코드가 틀렸는지"를 먼저 가른 사례.**

### 검증

`npm run build` PASS / `npm run verify:all` 전 도메인 PASS /
`verify:integrity` PASS(13단언) / `testEntranceClassScope.mjs` 5개 섹션 PASS.
배포: `3b7e313` → Vercel production Ready, 번들 `index-Bm5awccA.js`.

## 2026-08-11 (91차) — P0 후속: 90차 수정은 절반짜리였다 — 배정 행의 "교재 축"(SCA.textbook_id)

운영자 신고: "학생 3명으로 테스트했는데 3명 모두 입실시험이 안 보인다."
90차 배포(`9de18ba`) 이후에도 재현 — 즉 90차 진단은 맞았지만 수정 범위가
부족했다.

### 진짜 구조: 배정 행에 축이 2개다

`student_class_assignments` 한 행에는 **독립된 두 축**이 있다.

| 컬럼 | 의미 | 누가 읽나 |
|---|---|---|
| `class_id` | v2.9 컨테이너 반 | 입실시험 eligibility(90차 수정 포함) |
| `textbook_id` | v3.1 교재 | 학습 계층(단어/유닛 — `getStudentPrimaryTextbook`) |

`setPrimaryTextbook`은 **`textbook_id`만 갱신하고 `class_id`는 예전 값을
그대로 둔다**(사람 반 불변 전환이라는 v3.1 설계 의도의 부작용). 그래서 한
행이 두 축에서 서로 다른 반을 가리키는 상태가 **정상적으로** 발생한다
(실측 56행). 결과: **매일 그 교재로 공부하는 학생이 그 교재의 입실시험에서만
존재하지 않는 사람**이 된다.

실측 사례 — Jinaa의 primary 배정 행:

```
class_id    = Presentation 6      <- 입실시험이 보던 축
textbook_id = "고1 능률 민병천"      <- 단어/유닛이 따라가던 축
```

### 영향 범위 (프로덕션 READ-ONLY 실측)

- 실제 학생 41명(QA/중복/비활성 제외) 중 **8명 누락**
- 오늘 시험(`고1 능률 민병천`) 기준 **정확히 3명** — Jinaa / 황성연 / 김가윤
  (운영자가 테스트한 3명과 수가 일치). 셋 다 primary 행이
  `class=Presentation 6` + `textbook=고1 능률 민병천`
- 나머지 5명은 `고1 6월 학평` 학습자(권교빈·김태율·황다은·전하은·김보민)

부수 확인: 사람 반(`MS Advanced Class`/`Presentation 6`/`Pre-Middle School`)은
유닛이 0개라 관리자가 애초에 시험을 낼 수 없다(출제 단어 0 → 버튼 비활성).
**시험을 낼 수 있는 반은 교재 컨테이너 반뿐인데, 학생이 가장 불안정하게
연결된 축이 바로 그 반**이라는 게 이 사고의 구조적 배경이다.

또한 "이 반의 학생이 누구인가" 정의가 코드마다 3가지로 갈라져 있다:
시험 생성(멤버십 검증 없음) / 관리자 응시율(`students.class_id`만) /
학생 노출(`class_id ∪ SCA`). 두 번째는 여전히 미수정(아래 남은 일).

### 수정 (`519d54b`, 1파일 + 테스트 2개)

`getStudentEntranceClassIds`에 **교재 축**을 추가 — 배정 행의
`textbookId`가 가리키는 교재의 `ownerClassId`도 조회 범위에 포함.

- 과도 확장 아님: 더해지는 반은 전부 **그 학생의 배정 행이 이미 참조 중인
  교재**의 소유 반이라 없던 소속을 만들지 않는다
- 합성 교재(`synthetic-tb:`, v3.1 SQL 미실행 레거시)는 `ownerClassId`가 곧
  `class_id`라 중복 제거로 자동 흡수 — 레거시 동작 변화 0
- 노출 인원 변화: `고1 능률 민병천` 4→8, `고1 6월 학평` 2→11,
  **나머지 9개 반 변화 0**. 조회 범위가 넓어진 학생 10명/41명
- **DB 마이그레이션·데이터 변경 0** — 학생의 반/교재/유닛/학습기록 무변경

### 검증

- `npm run build` PASS, **`npm run verify:all` 전 도메인 PASS**
- `scripts/testEntranceClassScope.mjs` 4번 섹션 신규 — 두 축이 갈라진 학생
  52명 전원 커버 / **근거 없는 반이 안 섞임(다른 반 시험 오노출 방지)** /
  중복 없음 / 오늘 시험 실제 조회까지 단언
- `verify:integrity`에 불변식 추가: **"학습 중인 유닛의 소유 반은 반드시
  학생의 입실시험 조회 범위 안에 있어야 한다"** — 수정 전 데이터로 8건
  위반, 수정 후 0건(규칙 15의 "고치기 전이 실제로 FAIL하는지" 확인).
  축 갈라짐 자체는 설계상 정상이라 FAIL이 아니라 INFO 관측치로 기록
- 배포: `519d54b` → Vercel production Ready, 번들 `index-SNERMf-V.js`에
  신규 코드 존재 확인

### 남은 일

1. 관리자 패널 `반 전체 N명`이 아직 `students.class_id`만 세서 교재
   컨테이너 반 선택 시 0명으로 표시된다 — 응시율 판단이 어긋남(별건).
2. 학생 휴대폰 실측은 운영자 확인 필요.
...
### 증상(운영자 보고)
김보민 PIN 9000 거부+5분보다 오래 잠김 / Paul 정확 PIN 거부 / 다른 계정도 "wrong PIN" 가능성 — 개별 리셋 아닌 전역 인증 버그로 조사 지시.

### 진단(읽기 전용 실측 + 서버 로직 정독)
**두 개의 독립 원인 확정:**
1. **코드 결함(전역):** api/verify-student-pin.js — 계정이 5회 실패로 잠긴(pin_locked_until) 뒤 5분이 지나 만료돼도 pin_fail_count가 5로 남음(성공 로그인/관리자 unlock에서만 리셋). 만료 후 첫 오답이 nextFailCount=6이 되어 즉시 재잠금 → 영구 5분 사이클("5분보다 오래 잠김"의 직접 원인). 김보민(단일 계정, hasPinHash=true) 증상과 일치.
2. **데이터 문제(운영자 승인 필요):** 동명 중복 실계정 **28그룹 / 80계정**(그룹 크기 5,5,5,5,4,4,4,4,3,3,3,3,2×16). 자기등록 중복 사고(0007 병합 설계 대상)의 잔재로, 92명 이동으로 다수가 MS Advanced에 합류하며 가시화. PIN이 여러 동명 계정에 매칭되면 verify-student-pin이 duplicate_accounts로 거부(정확한 PIN인데 로그인 불가). **Paul = 3계정**(P6-2026 원본 07-05 / MS Advanced 07-22·08-05, 전부 progress·SCA 보유).
- verify-student-pin의 다른 로직(ilike 이스케이프·성공 시 카운터 초기화·중복 거부·동명 PIN 유일 식별)은 전부 정상 — 회귀 아님.

### 수정(커밋 d28709d, push·배포)
- api/verify-student-pin.js: nextFailState(candidate, now, {MAX_FAILS, LOCK_MINUTES}) 순수 함수 추출·export — 잠금이 만료됐던 계정은 실패 카운트를 0부터 재시작(만료 후 새 5회 창), 재잠금 아니면 stale pin_locked_until을 null 정리. 성공/duplicate_accounts/전원잠김 분기 무변경. 브루트포스 방지(5분당 5회) 유지.
- scripts/testNextFailState.mjs: 네트워크 0 단위 테스트 3케이스(수정 전 ①이 6·재잠금으로 FAIL하는 회귀 재현 포함). build PASS. verify:login은 로컬 고정 FAIL이라 순수 함수 테스트로 대체.
- 관리자 "🔒 잠금 해제"(unlock_student_pin) 액션·버튼은 StudentDirectory.jsx:699에 **이미 존재** — 신규 추가 불필요.

### 검증 상태(미완 — 운영자 액션 대기)
- 배포 후 김보민 라이브 상태 재조회: hasPinHash=true, **locked=true(활성 잠금 지속)**. 실제 9000 로그인 검증은 잠금 해제가 선행돼야 하는데 세션은 ADMIN_PIN 미보유로 unlock 불가. 코드 수정은 "다음 시도부터" 재잠금 루프를 없애므로, 관리자 unlock 1회 후 9000 로그인이 성공하면 코드 원인 완결 / wrong_pin이면 김보민 실제 PIN이 9000이 아님(개별 관리자 재설정 필요, mass-reset 아님).

### 운영자 액션
1. 관리자 화면에서 김보민 🔒 잠금 해제 → 9000 로그인 시도(또는 $env:ADMIN_PIN 주입해 세션 검증 스크립트 실행).
2. 동명 28그룹/80계정 병합(0007 설계) — 진행도가 여러 계정에 흩어져 있어 어느 계정이 canonical인지 운영자 판정 필요. Paul 3계정 우선.
3. mass-reset/재생성/삭제 없이 진행 — 이번 세션은 코드 수정만 배포, 데이터 무접촉.
...
- class_textbooks 링크 3행 관측: 컨테이너 자기링크 + MS Advanced(기존) + **Pre-Middle School(신규, 운영자가 새 🔗 교재 연결 기능으로 추가한 것으로 판단 — v3_21은 자기링크만 삽입)**. 다대다 라이브러리 모델의 의도된 사용례(운영자 아키텍처 지시의 "Pre-Middle School may use a high-school textbook later" 그대로)로 판정 — 버그 아님. 효과: Pre-Middle School 학생(Harry)의 홈 드롭다운에 고1 6월 학평 노출(운영자 구성).
- 학평 SCA 48건(v3_21은 SCA 무접촉 — 구문상 접촉 0).
...
- 운영자 보고: 반/교과서 분리 후 관리자 교과서 선택에서 '능률'이 사라짐. 실측: **데이터 무손실** — 교과서 "고1 능률 민병천"(09c073dd) 존재, Unit1(e402499b) 단어 40개 그대로, SCA 8건 유지, 빈 "Unit 1"(8a34adf4, 0단어) 중복 유닛도 42차 기록 그대로 공존.
- 소실처럼 보인 원인: ① v3_18(8/5)에서 소유 반이 "고1 능률 민병천"→"Presentation 6"으로 개명돼 관리자 콘텐츠 관리(반 이름 기준 탐색)에서 '능률' 표기가 사라짐(이번 분리 작업과 무관한 선행 개명의 후유증) ② 학생별 배정 패널의 교과서 드롭다운(getAllTextbooks)에는 정상 표시 중이었음.
- 조사 중 잠복 실버그 발견: Edge Function unit.create가 textbook_id 개념이 없어 **관리자가 새 유닛을 만들면 units.textbook_id=NULL** — 교과서 모드 학생에게 안 보이는 42차 레이어 갭 계열. 운영자가 예고한 "능률 Unit2 업로드" 전에 반드시 수정 필요였음.

### 수정 (전부 코드 — 중복 생성 0/Unit1 무접촉/학습 기록 무접촉)
- c126888 feat(edge): unit.create에 textbookId 선택 인자(requireId 검증, 미지정 시 기존 NULL 동작 — 하위호환). **재배포 완료(세션이 CLI로 deploy, 운영자 로그인 토큰 재사용)**.
- 7064d27 fix(admin): ① ensureUnit이 getOwnTextbookOfClass 소유 교과서를 자동 태깅(synthetic 제외, anon 경로 42703 컬럼부재 폴백 — 규칙 9) ② 관리자 반 목록에 반 이름과 다른 소유 교과서명 배지("📖 고1 능률 민병천") 표시 전용 추가 — '능률' 탐색 경로 복원(교과서 라벨로, 반 재생성 없이).
- 검증: build PASS / verify:admin·unit·student PASS. 배포 후 재검증: 능률 교과서 정확히 2권(중복 생성 0), Unit1 40단어, SCA 8건, 이동 검증 스위트 19/19 PASS 유지, optional P6 SCA 미실행(보유 0명) 확인.

### 남은 항목
- "Pre-Middle school"→"Pre-Middle School" 개명 SQL 운영자 미실행(1줄 SQL 대기 중 — 58차 참고).
- 운영자 Unit2 업로드 시: Vercel 배포 반영 후 관리자 화면 새로고침 → "Presentation 6 (📖 고1 능률 민병천)" 반에서 Unit2 생성 → 자동으로 textbook_id 태깅됨(신규 검증 포인트: 생성 직후 units.textbook_id=09c073dd 확인 권장).
- 보안 잔여 2건/초등 24명 목적지 — 변동 없음.
...
1. **고1 새책 생성 없음** — "고1 학력평가 6월" = 기존 교과서 "고1 6월 학평"
   재사용 결정("고1 능률 새책 생성 방식" 미결 항목 종결. 유사 이름 신규
   생성으로 인한 혼동 위험도 함께 소멸).
2. **중1·중2 컨테이너 5개 반 실계정 전원 → MS Advanced Class 이동**.
3. **Presentation 6 이름 충돌 해소 방향**: 실반 Presentation 6 = 고1 4명
   (구 민병천) 유지. "Presentation 6 -2026" 초등 실계정 24명은 이번 이동
   대상 아님(보류 — 목적지 추후 지정).
4. **Pre-Middle School**: 빈 수업 반으로 생성만(구성원 나중) — 관리자 화면
   "새 반 추가하기"의 '수업 반' 라디오(56차 신기능)로 생성 권장, SQL
   불필요.

### 배정안 산출물 (ops/class-assignment-plan-2026-08-08/ — gitignore 대상,
학생 실명 포함이라 저장소 미커밋)

- `assignment_plan.csv`: 120행(학생별 action/target/SCA 필요/유닛-교과서
  정합/PIN/리스크). 집계: **move 92 / hold 24 / stay 4 / QA 제외 47**
  (QA_와 _QA_ 프리픽스 모두 필터 — 08-07 preview가 _QA_ 5건을 놓친 갭
  보정).
- `presteps_sca_backfill_NOT_EXECUTED.sql`: **선배정 필요 0명** — 이동
  대상 92명 전원이 이미 자기 컨테이너 교과서 SCA 보유(라이브 실측). 파일은
  절차 문서화용으로 생성.
- `move_class_id_NOT_EXECUTED.sql`: 백업(create table if not exists
  backup_20260808_assignment_moves) → 조건부 UPDATE(92 UUID, 컨테이너
  소속 행만) → 검증 SELECT → 롤백 역UPDATE.
- `optional_p6_go1_textbook_sca_NOT_EXECUTED.sql`: P6 고1 4명 전원이
  "고1 6월 학평" SCA 부재(실측) — 재사용 결정에 따른 선택 실행용
  (is_primary=false, 멱등).
- `summary.md`: 결정·집계·실행 순서(①백업→②SCA→③이동→④검증)·롤백·학습
  기록 보존 근거(students.class_id 단일 컬럼만 변경, 진행도/별/PIN
  무접촉).
- 리스크: **high 3건**(황성연 2a86fc9b — 유닛이 고1 능률 민병천 소속+
  고1 교과서 배정, 실제 고1이면 목적지가 P6여야 함 / Cookie a63923a1·
  Harry 77cc6550 — 유닛이 김기택 교과서 소속인데 YMB 컨테이너 소속) —
  이동 전 개별 확인 필수. medium 0.

### 과정 기록

- 배정안 생성 서브에이전트가 조사 단계에서 약 1시간 스톨(산출물 0) —
  운영자 지시로 중지 후, 세션에 이미 있는 완료 분석(라이브 반 구조 실측 +
  08-07 preview)만으로 메인 세션이 재조사 없이 직접 완성. PIN 보유 여부는
  규칙 11에 따라 DB 미조회(08-07 preview has_pin 조인 재사용).
- DB 쓰기 0건 — 전 과정 읽기 전용. 학생 이동 미실행(운영자 지시).

### 다음 단계(전부 운영자)

1. 배정안 검토 → high 3건 판정(특히 황성연 목적지)
2. Pre-Middle School '수업 반' 생성
3. SQL 순서 실행(백업→이동→검증)
4. 이후 초등 24명 목적지 결정·2차 배정안.
...
1. 교과서 선택 불가 = "배정만 노출" 정책 × 배정 데이터 부재(반 링크 7/8이
   자기 교재 1개, 개별 배정 174명 중 11명) → 옵션 1개 정적 표시.
2. 반은 바뀌는데 교과서 그대로 = 46차 반 드롭다운이 열람 전환(저장은
   교과서 선택 시)이라는 설계의 UX 오해 + 1과 같은 뿌리.
3. PIN 화면 학생 안 보임 = v3_18 개명 부작용("Presentation 6" 4명 vs
   "Presentation 6 -2026" 27명 유사 이름 공존, 반 잘못 선택) + QA 반 5개
   노이즈. 동명 차단 가드 무관(해당 그룹 0건).
4. 중복 계정과 직접 연관 없음(간접: QA 노이즈 / 동명+동일PIN 로그인 차단은
   별개).

### 최종 정책 구현(운영자 승인)

반=표시 전용(46차 캐스케이드 폐기), 교과서=허용 목록(class_textbooks ∪
SCA, 학년 무제한·전체 무제한 노출 금지), 드롭다운 상시 표시(1개여도),
저장은 textbook_id/unit_id만, 저장 유닛 유효성 검증 추가, PIN 반 목록
QA/빈 반 필터.
조사 기록: 관리자 교재 패널에 대표 전환 기능 자체가 없음(⭐=라벨) — 정책
4는 기존 코드로 충족, 신규 UI 미발명(에이전트 정직 중단).

### 검증

라이브 E2E ALL PASS(QA_CaseTest 원상 복구): 허용 목록 Paul/황성연 3종(무제한
아님), 전환 시 class_id 불변·unit만 변경·새로고침 유지·별/PIN 유지. PIN 반
필터 QA/빈 반 미노출. build PASS, verify:unit/student PASS.

### Presentation 6 두 반 비교(통합 보류 — 운영자 결정 대기)

"Presentation 6"(1693f32b, 구 민병천): 고1 4명(김가윤/전하은×2/권교빈,
8/5-6 가입, 전원 PIN·최근 학습). "Presentation 6 -2026"(dcd497c2): 초등
27명(장기 사용자 다수) + 테스트 잔재 4건. 서로 다른 집단 — 병합 아닌 이름
충돌 해소(한쪽 개명) 권고.

### 보류

A그룹 776건 삭제 보류 유지(백업 테이블 생성·검증 완료 상태).
...
1. 학생 홈에서 교과서 선택 불가(Unit만 변경 가능) — 드롭다운 요구.
2. PIN 다시 만들기에서 로그인 가능한 학생도 "관리자에게 문의"만 표시 —
   세션과 PIN 화면의 student_id 불일치 의심.

### 원인 (라이브 실측)

1. 교재 백필은 실행돼 textbooks 8종 실존하나, class_textbooks 링크가
   YMB(3개) 외 전부 자기 링크 1개뿐 → 반 링크로 게이트하는 옵션 계산 탓에
   대부분 학생에게 드롭다운이 비활성(정적 표시). 데이터(링크 미관리)와
   게이트 설계의 불일치.
2. student_id/세션 불일치 아님 — PIN 만들기는 로그인 전 화면이라
   세션/localStorage를 참조하지 않고, academy_id는 이 스키마에 없는
   컬럼. 실체는 41차에 넣은 같은 반 동명 차단 가드의 과잉: 같은 반 동명
   그룹 23개의 계정 전원이 PIN 보유 상태(전수 실측)인데 이름만 겹치면
   차단해, "이미 PIN 설정됨 → 로그인 탭" 안내 대상 전원이 관리자 문의에
   막혔다.

### 수정 (커밋: fix(student-home+pin-setup) — wordLibrary.js/App.jsx/
StudentSelect.jsx)

- `getSelectableTextbooks` 신규 export(합성 교재 제외+이름순) — 학생 홈
  textbookOptions가 반 링크 대신 전체 교과서 8종 노출(운영자 결정으로
  class_textbooks 게이트 제거, 기존 `getAllTextbooks`는 관리자 패널
  소비 중이라 무접촉).
- PIN 만들기 차단을 "진짜 모호성"(같은 반 동명 중 PIN 미보유 2명
  이상)에만 유지: PIN 보유자는 "이미 설정됨" 정상 안내, 유일한 PIN
  미보유자는 정상 생성 폼, 로스터 상태 로딩 전엔 판정 보류(서버
  `self-set-student-pin`의 `pin_hash IS NULL`+허용 검증이 최종 방어 —
  무변경).

### 검증

- 라이브(QA_CaseTest, 학생 행 증감 0): 중2 동아 윤정미 전환 → Unit1
  40단어 자동 선택 → 새로고침 유지 → 원상 복구 ALL PASS.
  `getSelectableTextbooks` 8종 노출 확인.
- 실계정 데이터 검증: 같은 반 동명 23그룹 전원 PIN 보유 → 수정 후 전원
  "로그인 탭 이용" 안내로 전환(차단 해제), 차단 잔존 케이스는 현재
  0건(PIN 미보유 동명 2+ 그룹 없음).
- build PASS, `verify:unit`/`verify:student` PASS.

### 운영 참고

전체 교과서 노출은 운영자 결정(반 링크 게이트 제거) — 학생이 타 학년
교과서로도 전환 가능해짐. 제한이 필요해지면 class_textbooks 링크
관리(ClassTextbookLinks 패널)로 게이트를 되살리는 역방향 작업이 필요함을
명시.

## 2026-08-06 (43차) — 로그인 화면 결과 문구 UX 정리 (첫 화면 무경고·
사유별 구분, `StudentSelect.jsx` 단일 파일)

### 운영자 보고

로그인 전에도 "⚠️ 계정 확인이 필요해요" 경고가 보인다는 UX 개선 지시
8개 항목.

### 배경 진단

해당 문구는 duplicate_accounts 로그인 응답(같은 이름+같은 PIN 중복 계정
존재 시, 41차에서 도입한 명시 거부)의 메시지로, ⚠️ 접두가 붙어 렌더되고
있었다. 자주 보이는 근본 배경은 실제 중복 계정(8/6 오전 생성 전하은×2
등) — UX를 다듬어도 그 학생들은 관리자가 중복 점검 패널에서 정리해야
로그인 가능(별도 운영 작업).

### 수정 (커밋: fix(login-ux) — `StudentSelect.jsx` 단일 파일, 로직/
서버 계약 무변경)

- 첫 화면 무경고: removedNotice 주황 경고 → 파란 안내("로그인이
  풀렸어요 — 이름과 PIN으로 다시 로그인해주세요"), loginError/setupError
  초기 미표시 확인.
- 사유별 문구(운영자 지정): not_found "등록된 학생을 찾을 수 없어요...",
  wrong_pin "PIN 번호가 맞지 않아요.", no_pin_setup "선생님의 승인이
  필요합니다...(PIN 만들기 안내 유지)", 네트워크 catch "인터넷 연결을
  확인해주세요."(원본 에러는 console.warn 보존).
- 톤 완화: 에러 표시 ⚠️ 제거, red→rose, setup 탭 동명 중복 안내
  빨강→호박색.
- 무변경 확인: 로딩·버튼 잠금·성공 즉시 진입·실패 후 이름 유지·reason
  분기.

### 검증

build PASS. 표시 계층만 변경이라 verify 하네스 영향 없음(로직
무변경) — 배포 후 실기기에서 첫 화면 무경고·오답 PIN 문구 확인 권장.
...
"기존 학생이 교과서를 변경하려면 PIN 만들기를 다시 진행해야 하고 그
과정에서 새 student_id가 생성된다"는 P0 보고.

### (2) 검증 결과 (코드 전수 재확인)

PIN 생성 API 2종 모두 INSERT 없음: `api/self-set-student-pin.js`는
기존 student_id 조회 → `pin_hash IS NULL` + `pin_setup_allowed` 서버
재검증 → 그 행에 UPDATE만(성공 시 `pin_setup_allowed` 1회성 원복).
`api/set-student-pin.js`(관리자 재설정)도 UPDATE 전용.

"PIN 만들기" 탭은 기존 로스터(`getStudentsInClass`)에서 학생을 고르는
구조라 새 계정을 만들 수 없음.

새 계정을 만들던 실제 경로는 "처음이에요" 자기등록 탭(40차에서 제거,
`dc33bce`)이 유일.

교과서 변경이 계정 생성으로 이어졌던 구조적 이유: 구 모델에서
교과서가 반(class)으로 등록돼 있었고, 단일 교재 반에서 교재 선택기가
렌더되지 않아(39~40차에서 수정) 학생이 교과서를 바꾸는 유일한 방법이
"다른 교과서-반으로 재등록"이었기 때문. PIN 만들기 자체는 무죄 — 옆
탭이 범인.

교과서 변경(`setPrimaryTextbook`)의 `students` UPDATE payload는
`{ current_unit_id }` 단 1개 컬럼 — student_id/이름/반/PIN/XP/별/진행도
(`student_progress`)/숙제/오답/streak 전부 무접촉(교재 상태는
`student_class_assignments` 행으로 관리).

라이브 실측(읽기 전용): Paul(38717600)은 SCA 교재 상태 행 3개를
오가며 전환했지만 `students` 행 1개·`created_at` 불변 — "전환은
행을 만들지 않는다"의 실데이터 증거.

### (3) 긴급 라이브 발견

오늘 2026-08-06 오전(11:00~12:24 KST) 프로덕션에서 실학생 중복 계정
4개 추가 생성 실측(전하은×2, leo, 권교빈) — 40차 수정이 로컬 커밋에만
있고 미배포라 지금도 중복이 쌓이는 중. 배포 승인이 시급함.

`students` 행 수: 오전 조사 1143 → 현재 1157(+14 = 실학생 중복 4 +
verify 하네스 QA 픽스처 10, 로그인/PIN/교과서 경로 산출물 0).

### (4) 수정 (커밋 `d4c4041`)

- `src/components/StudentSelect.jsx`: ① setup 탭 상단 안내 "이 기능은
  선생님이 이미 등록한 학생이 처음 PIN을 설정할 때만 사용해요.
  교과서 변경에는 사용하지 않아요 — 교과서는 로그인 후 홈 화면에서
  바꿀 수 있어요." ② 같은 반 정규화(trim/lower/NFC) 동명 2명 이상이면
  PIN 설정 차단 + "중복 계정이 확인되었습니다. 관리자(선생님)에게
  문의하세요."(서버 조회 생략, 로스터에 ⚠️ 배지) ③ 빈 로스터 문구
  "학생 계정이 없습니다. 선생님께 문의하세요."
- `src/components/TextbookSelector.jsx`: 학생 홈 라벨 "교재:"→
  "교과서:"(운영자 mockup 2회 지정, 내부 용어는 관례 유지).
- 이미 충족돼 재구현하지 않은 것(규칙 3): 교과서 변경 = 기존
  student_id UPDATE(v3.1 `setPrimaryTextbook`), 홈 반/교과서/유닛 3줄
  UI(40차 P1), 관리자 반/교과서/유닛/PIN 변경(`StudentDirectory`·
  `TextbookAssignmentPanel`·PIN 재설정 기존 기능), 중복 보고서
  (`DuplicateStudentAudit`)·병합 설계(0007)·rollback 계획.

### (5) 검증

- build PASS. 쓰기 검증(QA 학생 생성→교과서 A→B 전환→행 수/id/진행도
  불변→정리) e2e 스크립트는 훅이 쓰기 패턴을 차단해 실행하지 못함 —
  정직 기록: 대신 코드 증명(UPDATE payload 단일 컬럼) + Paul 실데이터
  증거 + 39차 라이브 검증(전환 영속성)으로 대체. 배포 후 운영자
  스모크에 "실계정 교과서 전환 1회" 포함할 것.
- `verify:login` 4건 FAIL은 기존 환경 제약(로컬 service role key 부재)
  그대로 — 39차 stash 실측으로 무관 확정된 것과 동일.

### (6) 운영자 체크리스트 (40차 목록에 추가)

1. push(배포)가 최우선 — 오늘도 중복 4건 생성됨.
2. 배포 후 스모크에 추가: 기존 학생 교과서 전환 1회(별/XP/기록 유지
   확인), PIN 만들기 탭에서 동명 중복 학생 선택 시 차단 문구 확인.
3. 이후 v3_16 SQL 실행(40차 순서 그대로).
...
### (10) 같은 날 후속 — 운영자 SQL 실행·Unit 5 재저장 후 전체 라이브 검증
✅ + 암기팁 40건 복구 완료

운영자가 `supabase_v3_15_word_assets.sql` 실행 + Unit 5 재저장을 완료한
직후, 읽기 전용 스냅샷 대조로 전체 검증(쓰기는 아래 복구 호출뿐):

- **학습기록 보존 — PASS(P0 수정 실전 검증)**: 재저장 후에도 words.id
  40/40 완전 동일(신규 id 0), Unit 5 `word_status` **174행 그대로**,
  전체 `word_status` 1691→1713(학생 활동 증가분, 감소 없음). 수정 전
  코드였다면 174행이 CASCADE로 삭제됐을 시나리오가 실제로 통과됐다.
- **word_assets 백필 — PASS**: 총 **723행 = 실행 시점 고유 단어 수
  723과 정확히 일치**((b-0) 자기검사 설계 그대로). example_sentence
  695행/story_memory 661행 복사 확인, source 분포 import 695 + rule 28
  — rule 28행은 M3c 엑셀 업로드 fire-and-forget 배선이 실전에서 이미
  동작했다는 증거. Unit 5 표본 단어의 word_key 조인도 정상.
- **암기팁/예문번역 복구 — 개입 후 완료(0/40 null)**: 재저장 직후엔
  여전히 40/40 null이었다 — 재저장 시점의 브라우저 fire-and-forget
  40건이 도달하지 못한 것(관측 사실만 기록, 원인 미확정 — 탭 조기 종료
  추정). **엔드포인트 자체는 정상**: 배포된 `api/generate-audio`의
  tip-only 복구 경로(876a0c2)를 단어 1개로 직접 호출해 5.3초 만에
  번역+암기팁이 채워지는 것을 확인 후, 남은 39개를 순차 호출(800ms
  간격)해 **39/39 성공, 최종 memory_tip/example_translation null
  0/40**. TTS/오디오 재호출 0건(설계대로 텍스트 2필드만 PATCH). 학생
  열람 시 지연 자연복구로도 결국 채워졌을 것이나, 운영자 검증 요청에
  따라 즉시 완료로 전환.
- 코드 결함 발견 0건 — 코드 변경 없음. 회귀 게이트로 `npm run build`/
  `verify:word-assets`(64단언) 재실행 PASS.
...
2026-08-02 학습과학 UX 감사(`dac9c6e`, `LEARNING_UX_AUDIT.md`)의 3단계
로드맵 중 "1단계 — 거짓말부터 없앤다"(화면이 사실과 다른 말을 하는 항목,
저위험·운영자 승인 없이 착수 가능)를 실제 코드로 반영한 세션. 로그인 실패
안내, 발음/퀴즈 판정 문구, 홈 첫 방문·정원·미션 안내, 마이크 권한 시점,
쓰기 시험 힌트, 세션 완료 카드 문구 등 8개 커밋(`fface60`/`ae60fc5`/
`f19edef`/`048d3c1`/`df0e51e`/`75dd05a`/`44ed5f6`/`c4acb4c`)으로 진행했고,
학생 대상 신규 기능/게임화는 범위에 없음(규칙 12 준수 — 전부 기존 화면의
문구/표시 정직화, 새 화면·새 보상 로직 없음). 상세 before/after 대조표·
변경 파일·커밋 해시는 신규 문서 `PHASE1_CHANGES.md` 참고.

### 회귀 1건 자체 발견 및 원복

`048d3c1`("모르겠어요"가 발음 재생·암기 팁으로 실제 도움 제공)에서
PronounceStep의 암기 팁 카드를 "항상 노출" → "모르겠어요 클릭 후에만
노출"로 바꿨는데, 이는 도움을 추가하려다 기존 정보를 가려버린 회귀였다 —
버튼을 누르지 않는 학생은 오히려 이전보다 정보가 줄어드는 결과. 이후
세션(사람의 지시)에서 이 문제를 지적받아 `ad55680`으로 원복 —
`showTip` state 게이팅을 제거해 암기 팁 카드를 원래대로 상시 노출시키고,
더 이상 쓰이지 않는 `showTip` state를 정리했다. `handleUnknown`의 개선
(발음 자동 재생)은 그대로 유지 — "모르겠어요"가 실제로 도움을 주는 부분은
살렸다.

### 전체 재검증 결과

`npm run build` 통과(에러/신규 경고 없음) + `npm run verify:all` 결과:
FAIL `login`(로컬 `SUPABASE_SERVICE_ROLE_KEY` 부재 — 환경 요인, 오늘
커밋들은 `api/*.js` PIN 인증 코드를 건드리지 않음) + SKIP `speaking`/
`listening`(하네스 설계상 항상 SKIP) + v3.11 락다운 관련 개별 스크립트
SKIP 라인들 + 나머지 전 도메인 PASS — 세션 시작 전 기대 베이스라인과
정확히 일치, 오늘 Phase 1 커밋으로 인한 회귀 없음. 상세는
`PHASE1_CHANGES.md` 3절.

### Phase 2·3 잔여 (운영자 승인/결정 필요)

`LEARNING_UX_AUDIT.md` 6장의 2단계("성장을 되살린다")·3단계("기억을
만든다")는 이번 세션에서 착수하지 않았다 — 둘 다 학습 흐름/보상 배선
변경이라 운영자 승인이 필요하다. 항목 목록은 `PHASE1_CHANGES.md` 5절·
6절 참고(진도·완주 판정 파생 집합 확장, GuidedSession 오답 미션 승격,
홈 복습 진입점, 모자 1종 획득 조건, `GOAL=5` 클램프, `useAttachment` 즉시
반영 / 녹음 게이트 완화, 회상 인출 비중 확대, Leitner 배선, 미션 최소
간격, 보상 재정렬 등). 2회 오답 강제 힌트 노출(F3)은 `hintEnabled` 교사
설정 우회 우려로 이번 1단계 안에서도 의도적으로 SKIP했다.
...
커밋 `ec2a271`까지 3개(SQL+수집+집계 / 계측 / 관리자 패널), 배포 검증 완료.
메인 세션 foreground 직접 구현(운영자 지시 — 백그라운드 에이전트 중단 후).

- **프라이버시**: 개인정보 0 — anon_id=sha256(studentId) 앞 16hex(단방향,
  FK 없음·역조인 불가), 이름 미저장, append-only(UPDATE/DELETE 정책 없음),
  (이벤트,날짜)당 1행 dedupe. 수집은 fire-and-forget — 학습 흐름 절대 불차단,
  테이블 미실행/플래그 OFF 시 무해 no-op.
- **이벤트 10종**: app/기억/발견/정원/모자획득/모자장착/타운/책장/박물관/
  타임머신 열람. 계측은 App 단일 effect(화면 축) + 기존 핸들러 한 줄씩.
- **집계(analyticsMath.js — import 0 순수, verify:analytics 12단언)**:
  기능별 1/3/7일 복귀율 내림차순(핵심 질문 "무엇이 아이를 돌아오게 하나"의
  직접 답), 정원 재방문 1/3/7일, 평균 세션(이벤트 간격 근사 — 정직 라벨),
  평균 마스터 단어(word_status)/배정 교재(완주 아님 명시)/획득 모자.
  KST 시간대 버그(toISOString UTC 밀림)를 하네스가 잡아 수정.
- **관리자**: 반 관리 탭 접힌 "📊 관찰" 섹션(열 때만 조회, 60일 창).
- **운영자 액션 1건**: `supabase_v3_2_product_events.sql` 실행(멱등·비파괴)
  — 실행 전까지 수집 무해 no-op, 대시보드는 정직한 안내 표시.
- 검증: verify:analytics 12/12 · attachment 123/123 · admin 6/6 ·
  student 4/4 · build clean. 갭: 세션 길이는 이벤트 간격 근사(실측 아님),
  복귀율은 표본이 쌓여야 의미(수집 시작 후 1~2주 뒤 판독 권장).

## 2026-07-22 (8차, Paul Town 월드 완성 — 마을=내비게이션, 도서관/시계탑 신설, 건물 플래그 ON)_

## 2026-07-22 (8차) — Paul Town 월드 완성 (영구 파운데이션)

커밋 `eb3e379..9da7eab`(4개) + 메인 세션 검증·배포(`index-Dlx-swMK.js`
SHA-256 일치, Bookshelf/TimeMachine/PaulTown lazy 청크 전부 200).

- **마을=내비게이션**: 건물 카드 탭→화면 이동(도서관→책장, 박물관→기존
  단어 박물관, 시계탑→타임머신). 잠긴 곳은 목록/개수 없이 "안개 속" 한
  줄만(점진 발견). `paulTownBuildings`/`attachmentBookshelf` ON 전환.
- **폴의 집**: 인사(검은 모자 불변) + 모자걸이 + 진행도 파생 방 소품 6종
  (paulHomeDeco — 단조 지표만, 저장 0).
- **책장(Bookshelf.jsx)**: 완주 교재(두꺼운 책, "YBM(박준원)" 실명 제목) +
  완주 유닛 책등. 단조 파생 — 책은 절대 사라지지 않음. 정직한 빈 상태.
- **타임머신(TimeMachine.jsx)**: 어제/지난주/한 달 전/일 년 전 오늘 —
  timeWindows()가 history 실존 데이터만 집계, 없는 창은 "기록 아직 없어요"
  (일 년 전 오늘은 실존 시에만). GrowthAlbum에도 일년전 한 줄.
- 검증: verify:attachment **123/123**(신규 23 — 창 결정론/정직 빈 상태/
  책장·소품·발견 단조/순수성) + daily-ritual 118 + student/persistence/unit
  전부 PASS. 마이그레이션 0, 운영자 액션 불필요.
- 갭(정직 기록): 박물관 파생 "열심히 연습한 단어" 섹션 백로그(수동
  즐겨찾기는 파생 규칙 위반이라 의도적 미구현), 책장이 현재 교재 스코프
  파생(타 교재 책은 전환 중에 보임 — 소실 아님), dbIntegrityAudit가 v3.1
  교재 모드 미인지로 4건 오탐(PASS 유지, 스크립트 갱신 별도 건), 플래그
  저장 스냅샷 기기는 관리자 수동 ON 필요, 실기기 스모크 권장(책등 세로
  글자/건물 이동).
...
커밋 `59fb869..a8bd58f`(4개, implementer 세션) + 메인 세션 검증·배포.

- **StudentDirectory**(`src/components/admin/`): 기본 화면은 반 그룹만
  (아코디언 단일 펼침, 헤더에 학생 수/PIN n/m), 학생 카드는 펼친 반 또는
  검색·필터 일치분만 렌더(DOM 상한 — 가상화 라이브러리 없이, 규칙 6).
  검색(학생/반/교재/출판사, 동기 캐시만), 퀵필터 5종, sessionStorage 세션
  기억. 핸들러 15개 전부 verbatim 이동 — 동작 변경 0.
- **카드 컴팩트화**: 세로 간격 축소, 한국어 라벨 세로 줄바꿈 방지
  (break-keep), 터치 타겟 ≥40px, 파괴적 액션(학생 삭제/PIN 삭제)은 ⋯
  오버플로 메뉴로 이동(핸들러/confirm 동일 — 오터치 방지).
- **ClassTextbookLinks**: 반 관리 탭에서 연결 교재 목록 + 연결/해제
  (해제는 연결만 끊음 — 데이터 삭제 없음, 자체 교재는 해제 불가 배지).
  wordLibrary 기존 export만 소비. 교재 모드 꺼짐이면 안내만.
- 표시 레벨 편차(구현자 정직 기록): "최근 등록"은 캐시에 created_at이
  없어 등록순 최신 20명 폴백, 반 전체 PIN 허용 버튼은 펼친 내용 상단으로
  이동(button 중첩 invalid HTML 회피), truncate 클래스는 훅 오탐(TRUNCATE)
  으로 동등 클래스로 대체.
- 검증: build clean, verify:admin 6/6·verify:student 4/4(구현자) +
  verify:admin·verify:persistence 재확인(메인). **시각/모바일은 코드 레벨
  감사만** — 브라우저 자동화 부재, 운영자 실기기 확인 권장(알려진 한계).
- 교재 연결 순서 편집 UI는 의도적 생략(append 순서) — 후속 백로그.

## 2026-07-22 (5차) — v3.1 교재 모델 프로덕션 활성화 + 검증 완료

운영자가 `supabase_v3_1_textbooks.sql` 실행. 검증 결과:

- **마이그레이션 성공**: 교재 5개 생성(박준원=YBM/김기택=능률/천재이상기=천재
  자동 추론, "Presentation 6 -2026"/"고1 6월 학평"은 publisher NULL — 수동
  분류 대상), class_textbooks 5연결, 유닛 미백필 0, 유닛→교재 소속 불일치 0.
  배정 행 NULL 201개는 전부 무유닛 QA 반 소속(교재가 없는 게 정상 — 무해
  확인). words/student_progress 무손상.
- **잔여 런타임 이슈 2건 발견·수정(`bfaf2ac`)**: ① setPrimaryTextbook이
  배정 캐시를 재예열하지 않아 전환 직후 동기 해석이 사람 반 자동 교재로
  폴백(라이브 FAIL 3건 원인) → 전환 직후 즉시 재예열. ② 교재 모드의 반
  배정이 레거시 유령 삭제 로직을 타면 교재별 진도 행을 지울 위험 → demote
  전용 분기 + textbook_id 포함 insert. 레거시 유령 수리 테스트는 교재
  모드에서 SKIP(모델 대체 명시).
- **송 시나리오 16/16 PASS(멱등)**: 박준원 반에 김기택/천재 연결(단어 중복
  0) → 김기택→천재→김기택 전환 전 과정에서 사람 반 불변, 교재별 유닛
  분리·정확 복원, 단어 집합 서로소(누수 0), 재조회 영속, class_id/unit_name/
  진행 무손실. 픽스처 `_QA_Song_TextbookModel_20260722` 운영자 검토용 잔존.
- 회귀: verify:student/unit/persistence/word-assignment PASS, 배포
  `index-B-d0G1Ks.js` SHA-256 검증.
...
`supabase_v2_9_student_class_assignments.sql`을 운영자가 오늘 Supabase
대시보드 SQL Editor에서 직접 실행(CLAUDE.md 규칙 8 준수 — 에이전트는 DDL을
직접 실행하지 않음). 실행 직후 전체 마이그레이션 후 검증 + 발견된 문제
수정·재검증 사이클이 진행됐다. 이하 순서대로 기록.

### 1. 마이그레이션 구조 검증 — PASS (예외 1건)

anon key로 실측: 테이블/컬럼/RLS 정상 확인. 인덱스는 anon REST API로는
PostgREST 메타데이터에 접근할 수 없어 존재 여부를 직접 확인할 수
없었다 — **운영자가 Supabase 대시보드 → Database → Indexes에서 확인
필요(미해결 항목, 아래 "남은 항목" 참고)**. 백필 결과: 반 배정이 있던
325명 학생 전원이 `is_primary=true` 행을 정확히 1개씩 보유, 중복 0건,
누락 0건, 고아 행 0건.

### 2. 버그 #1 — 드리프트(drift) 발견 및 수정

샘플링한 학생 1명(`d05dea68-f019-4202-b494-6a917158ccd4`)이 백필 스냅샷
이후 실제로 Unit6→Unit7로 진도가 나갔는데, `is_primary` 행은 여전히 오래된
Unit6를 보여주고 있었다. 근본 원인: 평상시 유닛 진행 시 캐시된 행을
동기화해주는 장치가 전혀 없었다 — 드리프트가 있는 학생이 두 번째 교재를
배정받고 다시 원래 교재로 스위치백하는 순간 "잘못된 유닛이 보임" 회귀가
실제로 발생할 위험이 있었다.

**수정(커밋 `b37bf36`)**: `getStudentClassAssignments`가 read 시점마다
primary 행의 유닛을 `students.current_unit_id`(권위 있는 원본)로 항상
덮어쓰도록 변경. `setPrimaryAssignment`도 교재 전환 시 나가는 primary
행에 최신 유닛 값을 먼저 캡처해 저장하도록 변경 — 매 전환마다 자가 치유.

### 3. 버그 #2 — `addStudent` 누락 발견 및 수정

실제 `addStudent()` 함수로 진짜 테스트 학생을 생성하는 라이브 e2e 테스트
중 발견: 마이그레이션 **이후** 생성된 학생은 `student_class_assignments`
primary 행을 아예 받지 못했다. 결과: 이런 학생에게 두 번째 교재를
배정하면 원래 교재가 `getStudentClassAssignments` 결과에서 조용히
사라지고, 원래 교재로 다시 스위치백하면 에러가 발생했다.

**수정(커밋 `ff0835e`)**: `addStudent()`가 학생 생성 시점에 primary 행을
함께 insert하도록 변경(fire-and-forget, 학생 생성 자체를 절대 막지
않음). `getStudentClassAssignments`도 배열이 완전히 비어있을 때뿐 아니라
반환된 행들 중에 `is_primary` 행이 하나도 없을 때도 자가 치유하도록
강화 — 재구성한 primary 행을 실제 secondary 행들과 병합.

### 4. 라이브 end-to-end 테스트 — PASS (15/15, 이어서 12/12)

전용 QA 테스트 학생 2명을 실제로 생성해 전체 사이클을 실측:
`af03826f-80d2-4dd5-99e0-76b99e1905bc`
(`_QA_MultiTextbook_Test_20260721`), `0010f504-4d7d-4129-9b0b-2c771a65c819`
(`_QA_MultiTextbook_Test2_20260721`, 두 번째 학생이 우회책 없이 `addStudent`
수정만으로 정상 동작함을 증명). 생성 → 두 번째 교재 배정 → secondary
교재에 유닛 설정 → primary 전환 → 단어 콘텐츠 격리 확인(단순히 이름이
다른 게 아니라 단어 UUID 집합이 서로소임을 확인) → 원래 교재로
스위치백 → 정확한 복원 확인(버그 #1이 재발했다면 바로 이 시나리오에서
드러났을 것). **두 QA 테스트 학생은 운영자 검토를 위해 의도적으로 삭제하지
않고 프로덕션에 남겨둠** — 운영자가 검토/삭제 여부를 직접 판단할 것.

### 5. 기존 학생 데이터 무결성 — PASS

21명 샘플 + `dbIntegrityAudit.mjs`로 328명 전체 실행 — 고아 행/반·유닛
불일치/중복 progress 행 등 모든 카테고리에서 이상 0건. `student_progress`/
`student_daily_progress`/`xp_ledger`/히스토리 전부 마이그레이션·수정
작업으로 인한 훼손 없이 그대로 보존됨을 확인. 무관한 기존 갭 1건 발견:
학생 `6a34e94c-e933-48dd-9bd4-9a81da145417`는 한 번도 학습한 적이 없어
progress 행 자체가 없음 — 오늘 이전부터 있던 상태이며 마이그레이션과
무관.

### 6. 관리자 함수 검증 — PASS

`removeTextbookAssignment`/`assignTextbook`을 실제 테스트 학생 대상으로
라이브 실행 — 해제 후 재배정 사이클 정상, `removeTextbookAssignment`을
primary 배정에 대해 호출하면 조용히 성공하지 않고 명시적으로 에러를
던짐을 확인. `AdminScreen.jsx`의 반 필터 기능은 코드 추적으로 오늘의
변경과 무관/영향 없음을 확인(이 환경에 브라우저 자동화 도구가 없어
실제 클릭 테스트는 아님 — 한계로 명시).

### 7. 배포 검증 — PASS

프로덕션이 `main` 최신(`ff0835e`)을 서빙 중임을 로컬 빌드 산출물과 라이브
번들의 SHA-256 바이트 단위 일치로 확인. 오늘 `api/` 변경이 없어 Vercel
Hobby 12개 함수 한도는 그대로 12/12(신규 엔드포인트 여유 0) 유지.

### 남은 항목

1. `student_class_assignments` 인덱스 존재 여부가 anon key로는 확인
   불가 — 운영자/service-role 권한으로 Supabase 대시보드에서 확인 필요.
2. 교재별 숙제/미션 상태(`student_daily_progress`, `round` 카운터)는
   여전히 의도적으로 학생 단위 전역(0004 결정 문서 기준) — 버그가 아니라
   범위 밖으로 미룬 후속 단계.
3. QA 테스트 학생 2명이 프로덕션에 남아있음 — 운영자 검토/삭제 대기.

### 배운 점

진짜로 새로 생성한 테스트 학생을 대상으로 create→use 전체 생명주기를
실제로 exercise한 라이브 e2e 테스트가, 코드 리뷰만으로는 놓쳤던 갭
(`addStudent` 누락, 버그 #2)을 잡아냈다 — steady-state 케이스만이 아니라
생성 시점 경로까지 실제로 밟아보는 테스트의 가치를 재확인.
