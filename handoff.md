...
## 2026-07-10 밤 (4차, 진행 중) — 관리자 CSV + 성능 최적화 착수 (커밋 `a05eaa0`)
...
### 진행 중: 성능 최적화 (Priority 2)
`npm run build`가 매번 "메인 청크 879KB" 경고를 냄 — 학생이 가장 많이 쓰는 학습 화면 로딩에 관리자 전용 무거운 라이브러리(xlsx, PDF 생성 등)가 같이 딸려가고 있는지 확인 중.
