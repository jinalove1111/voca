...
### 2026-09-13 야간 추가 — V2-B 진행 반영

같은 야간 세션에서 코드 담당 에이전트들이 위 1.1(바텀시트 접근성 —
Escape 닫기·포커스 트랩·body 스크롤 잠금·닫을 때 트리거로 포커스 복귀)
과 1.5(모바일 폴리시 중 safe-area inset), 그리고 배치 상태
aria-label(1.3 배치 UX 다듬기와 인접한 접근성 보강)에 **이번 세션에서
착수**했다고 전달받았다 — 다만 이 문서를 쓰는 docs-maintainer는 해당
코드를 직접 리뷰하지 않았으므로, 이 세 항목은 "완료"가 아니라
**예정/진행 항목**으로만 기록한다(실제 반영 여부·QA 통과 여부는
`handoff.md`의 코드 담당 세션 기록이 우선한다).

남아 있는 항목: 1.2(구매했지만 미배치인 아이템의 소포 더미 표현,
`ui/parcel`), 1.4(실제 에셋 통합 배선 그 자체 — 이번 세션에 새로 작성한
`V2B_ARTWORK_DROPIN_CONTRACT.md`가 이 항목의 파일/폴더/검증 계약을
별도로 문서화했으므로 그쪽을 참고), 1.6에서 이미 식별된 죽은 props
정리(원문 그대로, 이번 추가로 새로 발견된 항목 없음)는 여전히
미착수다.
