...
| `decorations/bench` | bench.webp | decorations | 72×48 | 144×96 | 3:2 | sm | 소형 | 우드브라운+머티드네이비 옅은 악센트(#8b6f3e,#1e2a5a) | NO | **없음**(램프/간판/책/꽃/화분/새/포장/배경 장식 전부 금지 — 벤치 단독) | 해당 없음 | 1 | 15 | 없음 | **REGEN_REQUIRED**(bench1/3/4/5/6/7/8.png — 8차 재제출까지 전부 동일 계열 결함. bench5/6은 벤치+가로등 2오브젝트 합성이라 단일 오브젝트 요건도 위반(6=5와 바이트 동일 중복). bench7/8부터 단일 오브젝트 구도는 정확해졌고 종횡비도 계약(3:2)에 근접했으나(bench7 실측 1.46), 캔버스 전역에 걸친 은은한 방사형 글로우/비네트는 8차까지 전혀 해결되지 않음 — crop/repad로 안전하게 제거 불가, 배경 없는 순수 컷아웃 재출력만이 유일한 해법(§3 "조사했지만 자동화 불가" 참고)) |
