...
## 9. 진행 현황 갱신 (addendum, 2026-09-14 기준)

PR #49/#50/#51이 병합되며 아래 1건이 이 문서 작성 시점 가정과 달라졌다
(append-only 원칙에 따라 위 표는 그대로 두고 여기만 정정):

- **#2행 정원 4x4/16칸 재확인**: "V2-A는 아직 16칸 격자 시각 자체는
  없다"는 서술은 더 이상 정확하지 않다. PR #50(`9b72808`)이
  `gardenStageSprite()`를, PR #51이 실제 Batch-1 아트
  (`nature/garden-stage-0..4.webp`)를 연결해, V2는 이미
  `gardenRichness()`의 5단계(0/10/30/60/100)를 단일 배경 스프라이트로
  렌더 중이다(`TownAmbientLayer.jsx`, `paulTownV2` 플래그 뒤, 오늘도
  OFF라 학생 화면 영향 없음). 즉 "정원 생기"의 시각적 흡수는 16칸
  격자 축소 렌더가 아니라 **richness 5단계 단일 스프라이트** 방식으로
  이미 사실상 결정·구현됐다 — `V2B_V2C_ROADMAP.md` 2.2절의 "디자인
  결정 필요" 문구는 이 코드로 사실상 해소됐으니 그 문서 쪽에도 같은
  취지의 addendum을 남긴다(아래 참고). 남은 진짜 미정 사항은 "당일
  gardenPoints 변화를 당일 반영하는 시각 피드백"뿐(2.2절 계속
  유효, `V2C_LEARNING_VISIBLE_GROWTH_DESIGN.md` 참고) — 격자냐
  스프라이트냐는 더 이상 열린 질문이 아니다.

나머지 판정(1, 3~12행)과 라우팅(3절)·리스크(7절)는 재확인 결과 여전히
정확하다 — 건물 탭 라우팅(7행/4절), retroWelcome 흡수(4행),
`townDiscovery.js` 재사용(8행, PR #44 여전히 OPEN)은 전부 아직
착수 전이다.
