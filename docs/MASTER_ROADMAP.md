...
5. **`docs/audit/2026-07-24-deployment-scale.md`(deployment-engineer,
   ...
   - **[신규, 가장 심각] Vercel Hobby 플랜은 ToS상 비상업적 용도
     ...
     명시적으로 경고합니다].
     **[2026-07-24 후속 확정]** 조정자가 별도 세션에서 WebSearch로
     확인 — [추정] 아님, **확정**. Vercel 공식 ToS가 "you shall only
     use the Services under a Hobby plan for your personal or
     non-commercial use"로 명시적으로 Hobby를 비상업적 용도로
     제한하고, 실제로 Hobby 플랜에서 발견된 상업 프로젝트에 대해
     경고/정지 집행 사례도 있음(출처: https://vercel.com/legal/terms,
     2차 확인 https://www.promptstoproduct.com/vercel-free-tier-limits).
     직접 원문 WebFetch 열람은 훅에 의해 차단돼 못 했으나 WebSearch
     결과로 취지가 명확히 확정됐습니다. 이 확정 사실은 §5 표 항목
     3b/19와 §9 출처에 반영했습니다.
...
| 3 | GitHub Pages 활성 여부 운영자 확인 → 활성 시 즉시 비활성화 | **P1(조건부) — [2026-07-24 후속] 부분 완료, 잔여 액션만 운영자 대기** | §0-2/§2-2 — 활성 상태라면 API 없는 그림자 배포가 실사용자에게 지금도 노출 중일 수 있음. `docs/audit/2026-07-24-deployment-scale.md`(6)도 독립적으로 활성 여부를 확정 못 해 동일 조치를 권고(§0-5). **후속 확정**: 조정자가 WebFetch로 `https://jinalove1111.github.io/voca/` 직접 열람 — 실제로 활성 상태였음을 확인(로그인/PIN 검증 전체가 깨진 채 라이브 노출 중이었음). 커밋 `c34a5e3`로 `.github/workflows/deploy.yml` 트리거를 `on: push`→`on: workflow_dispatch`로 전환, 향후 자동 재배포는 차단 완료 | 소(자동 재배포 차단은 완료, 남은 건 Settings 클릭뿐) | **남은 액션**: 운영자가 GitHub 저장소 Settings → Pages → Source를 None으로 변경해야 기존 게시된 사이트 자체가 완전히 내려감(자동 재배포 차단만으로는 이미 게시된 정적 페이지가 자동으로 내려가지 않음) |
| 3b | Vercel Hobby 플랜 ToS(비상업적 용도 조항) 최신 원문 직접 확인 | **P1(사실확인) — [2026-07-24 후속] 완료, 확정됨** | `docs/audit/2026-07-24-deployment-scale.md`(§0-5) 신규 발견 — 원래 [추정]이었으나 **후속 확정**: 조정자가 WebSearch로 확인, Vercel 공식 ToS가 Hobby를 "개인 또는 비상업적 용도"로 명시적으로 제한하고 실제 계정정지 집행 사례도 있음(출처: https://vercel.com/legal/terms). 20학원 상업 SaaS 운영이 기술적 용량 문제보다 **약관 위반으로 인한 서비스 정지**를 먼저 유발할 수 있다는 판단이 이제 사실로 확정됨 | 소(WebSearch로 완료, 원문 WebFetch 직접 열람만 훅 차단으로 미완료 — 취지 확정에는 지장 없음) | **완료됨. 이 확인 결과가 항목 19(Vercel 유료 전환)의 시급성을 P4에서 앞당기는 게이트를 실제로 트리거함** — 아래 항목 19 재평가 참고 |
...
| 19 | Vercel Hobby→유료 전환 또는 대안 검토 | **[2026-07-24 후속] P4 → 격상, 조건부 트리거가 실제로 발동함(P2~P3급으로 재평가)** | `handoff.md` 다수 세션에서 반복 기록된 "함수 12/12, 여유 0" 제약(deployment-scale 감사가 실측 재확인). **신규 근거**: 같은 감사가 발견한 Vercel Hobby ToS 비상업적 조항 — 원래 [추정, §0-5]이었으나 항목 3b가 WebSearch로 **확정**됨(Vercel 공식 ToS, https://vercel.com/legal/terms). **재평가**: 이 항목은 더 이상 "학생 수 증가로 인한 기술적 필요"가 아니라 "약관 위반으로 인한 서비스 정지 회피"이며, §5/§6/§7이 조건부로 미리 적어둔 격상 조건이 지금 실제로 트리거됐습니다. 다만 20학원으로의 실제 상업적 확장이 아직 사업적으로 확정된 것은 아니므로(§7이 명시한 "확장이 사업적으로 확정된 경우에만" P4 착수 원칙과 균형), **현재(단일 학원, 111명 규모, 상업 확장 미확정)는 즉시 강제 전환까지는 아니되, "20학원 상업 SaaS로 확장하는 순간이 아니라 그 확장을 사업적으로 결정하는 시점에 플랜 전환을 반드시 선행 조건으로 두어야 한다"는 점이 이제 리스크가 아니라 확정된 약관 사실로 격상됐습니다. 6개월 로드맵(§6)의 "0~1주차 항목 3b 결과에 따라 조기 편입" 조건이 실제로 충족됐으므로, 운영자가 20학원 확장을 사업적으로 결정하는 즉시(§7의 "확장 확정" 게이트) 이 항목은 P4 대기열에서 최우선으로 꺼내 착수해야 합니다 | 소~중(플랜 변경 자체는 소, 이후 함수 통합 관례 유지 필요) | 확장 결정(사업적 확정 필요) — 단 항목 3b(ToS 확인)는 완료됨, 남은 의존관계는 확장 결정뿐 |
...
**[2026-07-24 후속 확정 근거 추가]** 조정자가 별도 세션에서 확인:
- WebSearch, 확인 대상 https://vercel.com/legal/terms(공식 Vercel Terms
  of Service) — Hobby 플랜의 "personal or non-commercial use" 제한
  문구 및 상업 프로젝트 계정정지/경고 집행 사례 확인. 2차 확인
  https://www.promptstoproduct.com/vercel-free-tier-limits. (직접 원문
  WebFetch 열람은 이 확정 세션에서 훅에 의해 차단됨 — WebSearch 결과로
  취지 확정.) §0-5/§5 항목 3b/19 갱신 근거.
- WebFetch, `https://jinalove1111.github.io/voca/` — GitHub Pages
  그림자 배포가 실제로 활성 상태였음을 직접 열람으로 확인. 조치 커밋
  `c34a5e3`("fix(deploy): GitHub Pages 그림자 배포 자동 재배포 중단").
  §5 항목 3 갱신 근거.
