...
> **갱신(2026-07-20)**: `bulk-generate-temp-pins.js`는 이후 `set-pin-setup-allowed.js`/`unlock-student-pin.js`와 함께 `api/admin-pin-actions.js`(단일 파일, `action` 필드 기반 dispatch)로 통합됐습니다 — 원본 문구는 그대로 유지하되(파일 3개가 각자 무엇을 했는지 이해하는 데 여전히 유효), 실제 요청 경로는 `POST /api/admin-pin-actions`(`action: 'bulk_generate_temp_pins'`)입니다. 통합 배경/이유는 8번 섹션 "배포 프로세스"와 `handoff.md` 2026-07-20(2차) 참고. `set-pin-setup-allowed.js`/`unlock-student-pin.js`도 동일하게 이 단일 파일 뒤로 옮겨졌습니다(`action: 'set_pin_setup_allowed'`/`'unlock_student_pin'`).
...
- **갱신(2026-07-21) — 위 "관계 골격" 1—N 가정은 조건부로 바뀝니다.** `classes`/`units` 1—N `students`는 여전히 "학생은 반이 하나"만 전제하는 기존 15개 이상 호출부(`getStudentClassId`/`getStudentWords` 등, `wordLibrary.js`/`AdminScreen.jsx`)에서는 계속 사실이지만, 신규 조인 테이블 `student_class_assignments`(다대다)가 도입되면서 학생이 **동시에 여러 반(교재)** 을 가질 수 있는 경로가 코드 배포까지는 완료됐습니다(DB 마이그레이션은 아직 미실행 — 그 전까지는 이 절의 원문 그대로가 실제 동작입니다). 상세 컬럼/RLS/마이그레이션 상태는 새로 만들지 않고 `DATABASE.md`의 `student_class_assignments` 섹션을, 설계 근거는 `docs/agent-decisions/0004-multi-textbook-architecture.md`를 참고하세요 — 여기서는 중복 기술하지 않습니다.
...
4. **Vercel Hobby 플랜 서버리스 함수 12개 한도 (2026-07-20 실제 사고 이후 명문화).** 이 프로젝트는 Vite(non-Next) 프레임워크라 `api/` 디렉토리의 파일(밑줄 접두 헬퍼 제외) 1개가 곧 서버리스 함수 1개로 직접 매핑되고, Vercel Hobby 플랜은 배포당 이 함수 개수를 12개로 제한합니다. 2026-07-19 세션 9차(House System)에서 `api/` 파일이 14개가 되면서 이 한도를 초과, **그 이후 모든 배포가 HTTP 에러 없이 조용히 실패**했습니다(신규 파일 push는 되지만 Vercel이 배포를 완료하지 못해 라이브 사이트가 계속 이전 번들을 서빙 — 로그를 따로 안 보면 알아채기 어려운 실패 모드). 2026-07-20에 관리자 PIN 액션 3개(`bulk-generate-temp-pins.js`/`set-pin-setup-allowed.js`/`unlock-student-pin.js`, 셋 다 같은 인가 게이트 `checkAdminReauth()`를 공유)를 `api/admin-pin-actions.js` 하나로 통합해 14→12개로 감축, 배포 정체 해소(상세: `handoff.md` 2026-07-20(2차)). **새 `api/*.js` 파일을 추가하기 전에는 반드시 `git ls-tree -r --name-only HEAD -- api/`(밑줄 헬퍼 제외)로 현재 개수를 먼저 확인할 것** — 12개에 근접했다면 새 파일을 추가하는 대신 같은 인가 게이트를 공유하는 기존 액션들을 `action` 필드 dispatch 패턴(`api/admin-pin-actions.js`가 선례)으로 먼저 통합하는 걸 고려한다. `DEVELOPER_GUIDE.md` "Deployment Checklist" 참고.
