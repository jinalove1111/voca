...
6. **새 `api/*.js` 서버리스 함수 파일을 추가하기 전, Vercel Hobby 플랜의 배포당 12개 함수 한도를 초과하지 않는지 확인한다** (`git ls-tree -r --name-only HEAD -- api/`로 밑줄 헬퍼 제외 현재 개수 확인). 2026-07-19 세션 9차(House System)에서 이 한도(당시 14개)를 초과해 그 이후 모든 배포가 에러 없이 조용히 실패한 실제 사고 사례가 근거(`ARCHITECTURE.md` "8. 배포 프로세스" 4번, `handoff.md` 2026-07-20(2차) — `api/admin-pin-actions.js` 통합으로 14→12개 감축해 해소). 한도에 근접했다면 새 파일을 추가하는 대신, 같은 인가 게이트(`checkAdminReauth()` 등)를 공유하는 기존 액션들을 `action` 필드 dispatch 패턴 하나로 통합하는 걸 먼저 고려한다.
