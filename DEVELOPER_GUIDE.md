...
_추가: 2026-07-18(개발자 인프라 구축 세션, Phase 3). 이 섹션부터는 append — 위 내용은 원본 그대로 보존._

새 AI 세션(사람이든 에이전트든)이 이 저장소에서 작업할 때 따라야 하는 6단계. `tests/harness/`(신규, `TESTING.md` 참고)가 생기기 전에는 4단계("관련 verify 하네스 실행")가 사람이 `buildXBundle.mjs`를 먼저 수동 실행해야 하는 번거로운 단계였으나, 지금은 `npm run verify:xxx` 한 줄로 축약됨.

1. **문서 읽기** — 작업 범위에 맞는 문서만 골라 읽는다(전부 다 읽을 필요 없음, 아래 매핑표 참고). 최소한 `PROJECT_GUIDE.md`(헷갈리는 것 Top 5)는 항상 먼저 읽는다.
   ```
   PROJECT_GUIDE.md          # 항상
   ...
   ```
2. **기능 구현** — `Development Rules`/`Coding Standards`/`Component Rules`/`Hook Rules`/`Database Rules`(이 문서 위 섹션들) 준수. 새 파일은 `Naming Convention` 표를 따른다.
3. **관련 verify 하네스 실행** — 건드린 영역에 해당하는 도메인만 우선 실행(전체는 마지막에 한 번):
   ```
   ...
   ```
   `speaking`/`listening`은 headless 환경 특성상 항상 SKIP으로 출력된다(정상) — 이 두 도메인을 건드렸다면 코드 리뷰 + 실기기 확인을 별도로 명시할 것(`TESTING.md` 참고).
4. **실패 수정** — 하네스가 FAIL을 보고하면 표준 출력(PASS/FAIL, 실패 위치 `파일:스크립트`, stdout tail)을 그대로 근거로 삼는다. 회귀가 의심되면 `Testing Rules`의 "수정 전 코드로 되돌려 FAIL 재현 확인" 패턴을 따른다.
5. **문서 갱신** — 아래 "아키텍처 변경 시 문서 갱신 규칙" 표를 참고해 관련 문서에 **append**한다(덮어쓰기 금지).
6. **git 요약 준비** — `npm run build` 통과 확인 → 변경 파일 목록 + 실행한 verify 도메인 + PASS/FAIL/SKIP 요약을 커밋 메시지/handoff 기록에 포함. 이 저장소는 파일/기능 단위 소커밋을 선호한다(`Development Rules` 2번).
