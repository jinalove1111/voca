...
v1.1(61a2c5a)의 관리자 패널 미노출 4번째 원인(flag가
`getFeaturesByCategory` 어느 카테고리에도 미편입돼 관리자 "기능" 탭에
토글 자체가 없었던 문제)까지 근본 수정하고, 캐시 키를 6필드로 버전화
(prompt/모델 포함)해 프롬프트·모델 교체 시 stale 캐시 오염을 방지, 서버
측 요청당 항목 수/비용 상한(`MAX_ITEMS_PER_REQUEST`/
`MAX_EST_COST_USD_PER_REQUEST`)을 신설. 커밋 `805c5d9`(파이프라인)·
`458e51a`(UI)·`1bf0fbc`(테스트, 섹션 37~49 총 198 PASS/0 FAIL/5 정직
SKIP). build PASS + verify:writing 3/3 PASS + Vercel 배포 SHA-256 검증
완료. 운영자 승인 하에 합성 데이터 라이브 E2E 1건(미리보기 무변경 실증
→ accept → 동일 답안 재분류 AI 0회 실증 → 100% 원상복구, 최종
143/15/48 일치) 수행 — 감사 테이블 append-only 잔존 행 1건은 운영자 SQL
1줄로 제거 가능. 운영자 액션 대기: v3_6/v3_7 SQL 실행, Edge Function
배포+시크릿, 관리자 화면에서 flag ON. 자동 일괄 승인 기능은 존재하지
않음(미리보기+수동 확인 구조 유지). 상세: `handoff.md` 2026-07-23(8차).
...
지문/문장 스키마(유닛 소속 — 계층 자동 상속) + 순수 모델 + API + 관리자
지문 편집기. 학생 학습 UI는 의도적 제외(readingStudentUI 예약 플래그만).
운영자: supabase_v3_3_reading.sql 실행 → 관리자 유닛 관리에서 지문 입력
시작 가능. 상세: handoff.md 2026-07-23(2차).
