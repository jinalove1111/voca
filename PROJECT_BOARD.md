...
### [P1] 입실시험 결과 서버 재검증 없음 (보안 Medium)
- 근거: `handoff.md` 2026-07-18 "Production Readiness Phase 4 보안 감사"
  신규 발견 — `src/utils/entranceTestApi.js:126`(`submitEntranceResult`),
  `supabase_v1_8_entrance_test.sql:63-64`(anon 전체 CRUD RLS).
- 내용: 클라이언트가 계산한 점수를 서버 재검증 없이 그대로 저장 + RLS가
  `using (true) with check (true)`라 anon key로 임의 `student_id`/
  `test_id`의 점수를 조작 가능(재현 실측 완료, 데이터 변경 없이 확인).
- 근본 수정안(문서화됨, 미구현): `api/submit-entrance-result.js` 신설 —
  서버가 저장된 `entrance_tests.words`/`direction` + 클라이언트가 보낸
  원본 답안으로 `computeTestResult()`(이미 순수 함수, 서버 재사용 가능)를
  재채점 후 결과만 저장.
...
_(현재 없음 — implementer 완료 후 qa-reviewer/security-reviewer 검수 대기
카드가 여기로 이동)_
