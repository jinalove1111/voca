...
export default function StudentSelect({ onSelect, onAdmin, onParent, removedNotice, legacyNotice }) {
  ...
  // 2026-08-08 — 교과서 컨테이너 반 노출 차단을 wordLibrary.getRealClassNames로
  // 구조화(v3_19 SQL 실행 전=이름 allowlist 폴백, 실행 후=class_type
  // 구조 판별 — wordLibrary.js의 classifyRealClassNames 헤더 주석 참고).
  // PIN 만들기 반 목록은 실반만 노출한다. 주의: 교과서명 반에 아직 소속된
  // 학생(정리 전)은 이 목록에서 찾을 수 없다 — 관리자가 학생을 실반으로
  // 이동하면 나타난다(운영자 인지 하에 즉시 적용 지시).
  const setupClassNames = getRealClassNames()
