...
> **[2026-08-02 야간] 학생 경험 폴리시 세션**
>
> - **커밋 목록**(전부 `main`, 시간순): `788e5ac`(하네스 42501 정직 SKIP —
>   classes/units/words anon 쓰기 스크립트) → `7e6f733`(W1 퀴즈: 오답 탭
>   전진·칭찬 슬랭 정리·발음 버튼 즉시 활성·셔플 편향 제거) → `7956c17`
>   (W2 단어학습: 오답 탭 전진·보기 A-D 통일·단어 카드 발음 탭·셔플 교정
>   ·backLabel prop) → `7398550`(W3 세션·복습: 완료 카드 내일 예고·복습
>   재진입 버튼·검수 탈출구·리액션 순화·백 라벨) → `8551f4c`(W4 홈·진입:
>   마이크 문구·구세션 안내 분리·유닛 변경 확인·SpeedBtn 라벨·상점
>   피드백·세션 props 배선) → `b1e907a`(W5-A 진단 로그 DEV 게이트 +
>   testQuizStepReset 플레이키 수정) → `96b071e`(W5-B vendor manualChunks
>   분리) → `3a3c3fc`(커밋1 — v3.11 픽스처 쓰기 정직 SKIP 확장,
>   homework/persistence 계열).
> - **영역별 개선 요약**: 오답 탭 전진(퀴즈/단어학습/세션 3곳), 검수
>   탈출구(완료 카드), 내일 예고(완료 카드), 유닛 변경 확인(대시보드
>   `window.confirm`), 마이크 오류 문구 분리 + 구세션 안내 분리(홈), 셔플
>   편향 교정(퀴즈/단어학습), 보기 A-D 표기 통일(단어학습), 단어 카드
>   발음 탭 배선, 칭찬 슬랭 정리(퀴즈 리액션), 상점 구매 피드백 칩,
>   진단 콘솔 로그 DEV 게이트, 메인 번들 644kB → 289kB(vendor 청크 분리).
> - **정직 기록(규칙 18) — 커밋 B(`96b071e`) 경위**: 오케스트레이터는 이
>   항목(vendor manualChunks 분리)을 애초에 야간 보류(다음 세션으로 이월)
>   결정했으나, 이중 디스패치 레이스로 동일 작업이 원래 담당 에이전트에게
>   중복 지시되어, 그 에이전트가 원 지시문의 선택 조항("여유가 되면 W5-B도
>   진행")에 따라 실제로 실행·커밋했다. 사후 검증(빌드 2회 재실행 + preview
>   서버 스모크 4종 200 응답 확인) 결과 회귀 없음을 확인하고 그대로
>   수용했다 — 되돌리지 않음. 교훈: 병렬 디스패치 시 "보류 결정"과 "실행
>   중인 에이전트에게 전달된 지시"가 서로 다른 소스로 갈릴 수 있으므로,
>   보류 결정은 지시 취소가 실제로 전달됐는지까지 확인해야 신뢰할 수 있다.
> - **의도적 미실행**(범위 밖으로 남겨둠, 버그 아님): 보상 지급 로직 8건은
>   이번 세션에서 표시(문구/UI)만 다루고 지급 로직 자체는 무변경. 개인
>   리더보드는 `PAUL_PRINCIPLES`가 금지하는 항목이라 신설하지 않음.
>   `WordBrowser`의 off-by-one류 항목은 이번 세션 범위 밖으로 의도적으로
>   손대지 않음.
> - **남은 아이디어**(구현 안 함, 후속 세션 후보): 조용히 모드(항목 1-6),
>   GiftReveal 스킵(항목 3-6), 타임머신 명칭 재검토(항목 6-6).
> - **최종 게이트**: `npm run build` PASS(경고 0/신규 없음, 기존 청크 크기
>   경고만 잔존) + `npm run verify:all` — FAIL {login}(환경, 로컬
>   SUPABASE_SERVICE_ROLE_KEY 미설정으로 인한 기존 클러스터, 이번 세션
>   무관) + SKIP {speaking, listening}(하드웨어/청각 관측 불가, 기존) +
>   v3.11 락다운 SKIP 다수(정직 SKIP, 이번 세션 커밋1로 3개 스크립트 추가
>   커버) + 그 외 전 도메인 PASS.
>
